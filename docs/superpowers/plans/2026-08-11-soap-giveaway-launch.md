...
5. **Klaviyo list double opt-in — VERIFY, do not assume it needs doing.** `GET /api/lists/{id}/` returns `attributes.opt_in_process`. Verified live 2026-08-11: list `Y2ukbE` is already `double_opt_in`, with `created === updated` proving no human edited it — API-created lists appear to default that way. This field IS exposed, contrary to an earlier claim in this plan, in `lib/klaviyo-profiles.js`, in `scripts/giveaway/setup-list.mjs` and in the runbook. **Gate A must assert it** rather than leaving the campaign's most load-bearing prerequisite to a checklist. The account is not uniform — `S6hKFq "Email List"` is `single_opt_in` — so a re-created list could silently land single, which is exactly why this needs an assertion.

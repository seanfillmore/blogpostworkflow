...
| 6 | Welcome flow `UUa3Qk` filtered to exclude `gv_entrant` profiles | **DONE — verified via API 2026-08-14.** **⚠ 2026-08-31: `UUa3Qk` NO LONGER EXISTS.** A go-live replaced the Welcome Series with `V5fp5i` and the duplicate was deleted (archived at `data/archive/klaviyo-flows/UUa3Qk-welcome-series-retired-2026-08-31.json`). The filter DID carry over — both definitions were identical, verified before deletion. Do not chase this id; resolve the live Welcome Series by NAME. `build-nurture-flow.mjs golive` now checks the suppression automatically. `profile_filter` now carries a fourth condition group: `properties['gv_entrant']` / existence / `not-set`. It is its own group, so Klaviyo ANDs it with the three metric gates (Placed Order, Added to Cart, Checkout Started, each count = 0) rather than ORing it away. Belt and braces: the welcome flow triggers on list `S6hKFq`, while entrants are added to `Y2ukbE`, so it would not fire for them even without the filter. |

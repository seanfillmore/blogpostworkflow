...
| `pump-lotion-callout-2026-09-11/` | `body-cream-benefits.png`, replaced because its callout read "Thicker than a pump lotion" — the RSC lotion is a squeeze bottle and only the foaming hand soap has a pump (the same error PR #859 fixed on the how-to frame and the PDP body) | No — deleted from the product, so the CDN file is gone. The replacement is this file with "pump" excised, per `scripts/publish-pdp-frames-batch3.mjs`. |
...
| `retired-lotion-frames-2026-09-12/` | `lotion-mechanism.LIVE-CDN.png`, the "SIX INGREDIENTS / Built to absorb…" frame retired from `coconut-lotion` because `coconut-lotion-ingredients-pdp.jpg` (PR #878) makes the same six-ingredient point two slots earlier, with every ingredient named. This is Shopify's 1400px re-encode that was actually live; the original generated source is `generated-pdp-frames/lotion/lotion-mechanism.png`. The Field Guide's image that pointed at it now uses `coconut-lotion-ingredients-pdp.jpg` — product media, so it survives `build-digital-assets.mjs --refresh-images`, which the Files-library-only Amazon variant would not. | No — deleted from the product, so the CDN file is gone. |

...
//
// TIMEZONE, READ THIS BEFORE "FIXING" IT. config/giveaway.json stamps -06:00 and calls it
// store-local; the rules prose says "CT", which in August/September is -05:00. The two
// therefore describe closing instants one hour apart. That is pre-existing and is NOT
// reconciled here, because this module only ever exposes CALENDAR DATES ("September 14,
// 2026") — which the two files agree on exactly — and ad copy states a date, never a
// UTC instant. The one-hour skew matters to the close-entry cron, not to a headline.
// It is flagged for the operator rather than silently normalised.
//
...
/**
 * "2026-09-14T23:59:59-06:00" → "September 14, 2026".
 *
 * Reads the LITERAL date component of the timestamp rather than converting through a Date.
 * The offset in the file is store-local, so the calendar date a reader of the rules sees is
 * the one written in the string; `new Date(...).getMonth()` would return the runner's local

 ...
 *
 * IT READS `c.revenue`, THE 28-DAY ENTRY-PAGE FIGURE, AND THAT IS A DELIBERATE
 * DEFERRAL AS OF 2026-08-23 — not an oversight. On that date the $0-cluster gate moved
 * off entry-page attribution and onto what a category's PRODUCTS SOLD over 90 days
 * (`lib/cluster-revenue.js`). `clusters[]` now carries both:
 * `entry_page_organic_revenue` / `revenue` (unchanged, byte for byte) and
 * `product_organic_revenue` / `product_revenue_all_channels`. This function was NOT
 * migrated with the gate, for two reasons worth stating rather than leaving implicit.
 *
 * 1. It would silently reshuffle the ranking. The saturation ceiling is $200 in the
 *    window, which entry-page figures rarely reach and product figures usually do — on
 *    the 2026-08-23 production report `soap` reads $62.40 entry-page and $324.85 sold,
 *    `deodorant` $38.25 and $165.00. Migrating the field alone would push most
 *    categories straight to the 20/20 revenue cap, flattening the very component that
 *    is supposed to discriminate between them. The ceiling has to be re-derived on the
 *    new basis in the same change, and that is a separate piece of work.
 * 2. Nothing here is a gate. This score only RANKS briefs; it never kills one (see the
 *    header). A stale basis costs ordering, not work — which is exactly why it may wait,
 *    and exactly why it must not be changed by accident.
 *
 * `tests/lib/ad-brief-score.test.js` pins that the product fields are ignored, so
 * the day someone does migrate this, it is a decision and not a side effect.
 *
 ...
 * no history.
 *
 * NO MATCH AND A MATCH THAT CARRIES NO SIGNAL SCORE THE SAME, AND MUST. Found live on the
 * server 2026-08-17: every RSC cluster attributes $0 revenue right now (seo-impact's
 * organic-revenue numbers are directional only — see CLAUDE.md's snapshot note and
 * project_revenue_attribution_unreliable.md — so this is not surprising), which used to
 * fall through to the ordinary revenue formula and compute 0/10 + 0 = 0. That reads as
 * "this product is commercially worthless", which is not what $0 attributed revenue means
 * — it means the attribution has nothing to say, the same epistemic position as no match
 * at all. So: a matched cluster set with zero total revenue AND zero revenueDelta on every
 * match (no momentum signal in either direction) is treated as NO-SIGNAL and scores the
 * same neutral as no match.
 *
 * GENUINE NEGATIVE MOMENTUM IS NOT COVERED BY THAT AND MUST NOT BE. A cluster can carry $0
 * revenue in the window and still report a real revenueDelta — "body lotion" on the server
 * is exactly this: rev=0, delta=-25.2, meaning the window that just closed earned less than
 * the one before it. That is evidence, not silence, and laundering it into the same neutral
 * score as "we have no idea" would hide a real decline behind a shrug. So the NO-SIGNAL
 * branch only fires when NOTHING in the matched clusters moved — the moment any match
 * carries a nonzero revenueDelta, control falls through to the ordinary formula below,
 * where zero revenue and no growth bonus already score it at the very bottom of the range
 * (0), distinctly below the 12-point neutral. Ranked low, not laundered.
 */
export function scoreCommercial(productHandle, seoImpact) {
  ...
  const hasMomentum = matches.some(c => (Number(c.revenueDelta) || 0) !== 0);

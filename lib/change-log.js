...
export async function proposeChange({ slug, changeType, category, nowIso = new Date().toISOString() }) {
  ...
  if (status === 'forming') {
    // Register this proposal as an event in the forming window so the change is tracked.
    ...
    active.changes.push(eventId);
    atomicWriteJson(windowPath(slug, active.id, CHANGES_ROOT), active);

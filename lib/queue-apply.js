...
import { getPostMeta, getMetaPath, getContentPath, listAllSlugs, writePostMeta } from './posts.js';
import { validateCollectionSpec } from './collection-validation.js';
import { MIN_COLLECTION_PRODUCTS, seoCopyViolationsForItem } from './queue-autoapply.js';
...
export async function publishBlogRefresh(item, deps) {
  ...
  const postSlug = postMeta.slug || item.slug;
  writeFileSync(getContentPath(postSlug), refreshedHtml);
  // The body was replaced, so every earlier post-performance verdict describes
  // a page that no longer exists. `last_refreshed_at` is what restarts that
  // review clock (agents/post-performance). refresh-runner always stamped it;
  // this path — every queue-driven refresh, i.e. nearly all of them — never
  // did, so a refreshed flop stayed "Action Required" forever.
  const stamp = deps.writePostMeta || writePostMeta;
  stamp(postSlug, { last_refreshed_at: deps.now ? deps.now() : new Date().toISOString() });
  return { published: true };
}

/**
 * Thrown when stored queue copy makes a claim a cosmetic may not make.
 * Typed so a caller can tell a compliance REFUSAL (the policy working, on the
 * normal deferred-digest path) from a Shopify FAILURE (an error).
 */
export class SeoCopyClaimError extends Error {
  constructor(violations) {
    const words = [...new Set(violations.map((v) => `${v.field}: "${v.match}"`))].join(', ');
    super(`Refusing to publish: health-claim gate — ${words}. This is a cosmetic, not a drug; `
      + 'the stored copy was generated by an earlier run and cannot be regenerated here. '
      + 'Re-run the producing agent, or edit and re-approve the item.');
    this.name = 'SeoCopyClaimError';
    this.violations = violations;
    this.healthClaimRefusal = true;
  }
}

/**
 * The last line before a live write.
 *
 * `agents/queue-autoapply` already gates in its PURE planner, so on the
 * scheduled path this should never fire — that is the point. It exists for the
 * other caller: the dashboard's Approve button, where a human can push any
 * pending item live without the planner ever seeing it, and for any future
 * caller of `applyItem`. A gate that lives only in one of two paths is the
 * exact drift this module was extracted to prevent (see the header).
 *
 * Throws rather than returning a verdict, so a caller that forgets to check
 * cannot write anyway.
 */
export function assertItemCopyIsCompliant(item) {
  const violations = seoCopyViolationsForItem(item);
  if (violations) throw new SeoCopyClaimError(violations);
}
...
export async function applyItem(item, deps) {
  assertItemCopyIsCompliant(item);
  switch (item.trigger) {

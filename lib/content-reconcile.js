...
//   faqRegression()     decided AFTER the write, not predicted — see below.
//
// WHY `FAQPage` IS LOAD-BEARING, WHICH IS NOT OBVIOUS FROM THIS FILE
// ─────────────────────────────────────────────────────────────────
// `agents/legacy-rebuilder` runs daily from `scheduler.js` at `--limit 5
// --apply`, and its `isLegacy(slug)` is literally `!html.includes('FAQPage')`
// read from `content.html`. 36 mirrors carry FAQ JSON-LD that their live article
// does NOT (the schema was injected locally and never pushed). A plain
// live → local copy therefore hands `legacy-rebuilder` 36 fresh "legacy" posts
// and it starts paying for full pipeline rebuilds of live pages, unattended, at
// five a day — a spend nobody asked for, triggered by a reconciliation that was
// otherwise correct.
//
// The fix is to re-run `agents/schema-injector` on the reconciled body, which
// regenerates the schema FROM THE NEW PROSE (so a different-article mirror stops
// carrying the old article's FAQ answers) and restores the invariant
...
//
// `faqRegression` is a VERIFY, not a PREDICTION. Whether the injector emits a
// FAQPage depends on its own heading heuristics, and a second copy of those
// heuristics here would drift from the agent silently. So the caller writes,
// re-injects, and then asks this function whether `FAQPage` survived; if it did
// not, the caller restores its backup. Measured on this corpus: 35 of the 36
// regain it, and one (`best-natural-bar-soap-for-men`, whose live body has a
// single question heading) does not and is rolled back.
...
/** Does this body carry FAQ schema, in the one way `legacy-rebuilder` asks? */
export function hasFaqPage(html) {
  return String(html ?? '').includes('FAQPage');
}

/**
 * Did re-injection fail to restore FAQ schema the mirror had before?
 *
 * True is a rollback, not a warning: leaving it true is what enqueues a paid
 * `legacy-rebuilder` rebuild the next morning.
 */
export function faqRegression(beforeHtml, afterHtml) {
  return hasFaqPage(beforeHtml) && !hasFaqPage(afterHtml);
}

/**
 * Mirrors that are byte-pinned fixtures of a committed remediation plan.
 *
 * `scripts/remediate-ingredient-benefit-headings.js` and
 * `scripts/remediate-tea-tree-11-benefits-post.js` each carry `kind: 'file'`
 * plan entries naming an exact BEFORE literal inside these files, and their
 * tests assert those literals against the REAL repo file. Every one of those
 * entries is already applied to LIVE (verified 2026-08-24: 15 of 15 mirror
 * entries and 14 of 14 KEPT excerpts present in the live bodies), so pulling
 * live down would leave the claims fixed on both sides — but it would also make
 * 15 plan entries unfindable at once, and one of them
 * (`mirror-coconut-why-antibacterial-stale-draft`) can never match again by
 * literal, because that mirror is a different, older article than what is live.
 *
 * Retiring compliance-plan entries is a compliance decision. It does not ride
 * along inside a mirror reconciliation, so these are held and named instead.
 *
 * Pinned by a test against both plans' exported `PLAN`, so a plan that gains or
 * drops a file target cannot drift away from this list unnoticed.
 */
export const PINNED_MIRROR_SLUGS = Object.freeze([
  '11-benefits-of-incorporating-tea-tree-oil-into-your-everyday-life',
  'incorporating-vanilla-skin-care-into-your-beauty-regimen',
  'why-choose-coconut-skin-care-products',
]);
...
export function decideMirrorAction({ comparison, localHtml, liveHtml, pinned = false } = {}) {
  ...
  return {
    action: 'reconcile',
    hold: null,
    reinjectSchema: hasFaqPage(local) || /application\/ld\+json/i.test(local),
 ...
 * → verify → roll back if FAQ schema was lost.
 *

...
/**
 * Is this post inside the OUTAGE scope — the only scope allowed to run unattended?
 *
 * The `different-article` tier and nothing else. This is the narrower question
 * "is this post currently un-republishable?", not "has this mirror drifted?".
 * `agents/publisher` refuses a different-article push, so every refresh, link
 * repair, buy-box rebuild and schema injection aimed at one of these is silently
 * refused until somebody reconciles it — and the tier regrows on its own (0 →
 * 28 between 2026-08-24 and 2026-09-08, every one `both-moved`).
 *
 * WHY THE WARN BAND IS EXCLUDED, and it is the whole reason this can be a timer
 * at all. `content.html` is also the INPUT to legitimate work: `refresh-runner`
 ...
 * destroys the rewrite. That hazard lives in the warn band — the deepest real
 * refresh on record scores 0.775, just above `DIVERGENT_WARN_MAX`, and a deep
 * one could plausibly dip below it. It CANNOT live at ≤ 0.25: that would mean a
 * "refresh" discarding three quarters of the article's text blocks, which is a
 * different article by any reading. `DIFFERENT_ARTICLE_MAX` was chosen at three
 * times below a measured real refresh precisely so this distinction holds.
 *
 * So the timer takes the tier that is an outage and a 3x margin from the hazard;
 * the warn band stays a human decision, and it blocks nothing in the meantime.
 */
export function inOutageScope(comparison) {
  if (!comparison) return false;
  return comparison.tier === 'different-article';
}

...
import { isRejected } from './rejected-keywords.js';
...
export function triageOrphanBrief(keyword, {
  ...
  // THE DESTRUCTIVE PATH: a false verdict here makes the brief droppable by
  // `scripts/triage-orphan-briefs.mjs --drop-non-earning --apply`, which archives
  // paid research. It used to compare raw strings for EQUALITY and ignore
  // `matchType` entirely — because its caller flattened the entries to strings —
  // so 37 of the 39 live entries, which mean SUBSTRING, silently matched nothing
  // here while `content-strategist` honoured them. It now reads the one shared
  // rule, and accepts either shape so an old caller cannot silently under-match.
  if (isRejected(keyword, rejectedKeywords)) {

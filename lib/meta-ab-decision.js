...
/**
 * Which side of the dead-band a CTR delta falls on: `improved` | `flat` | `regressed`.
 *
 * Extracted from `decideOutcome` so there is ONE statement of the rule. The 5 AM
 * digest needs the same answer and had been deriving its own from `winner`
 * alone — which collapses `flat` and `regressed` into a single "Variant A wins",
 * so a REAL regression (coconut oil as deodorant, -1.26pp) and a delta of
 * exactly 0.00pp rendered identically, both as a win. That is the same
 * "second copy of the decision rule" this module already removed from
 * meta-ab-checker's report; a second copy in the digest is no better.
 *
 * `decideOutcome` still owns PRECEDENCE (confounded and underpowered are decided
 * before this is ever consulted). This answers only the dead-band question, and
 * a caller holding a stored `outcome` should prefer that over re-deriving one.
 */
export function classifyCtrDelta(delta, { regressThreshold = DEFAULT_REGRESS_THRESHOLD } = {}) {
  ...
  if (delta >= regressThreshold - EPS) return 'improved';
  if (delta >= -regressThreshold - EPS) return 'flat'; // dead-band, symmetric
  return 'regressed';
}
...
export function decideOutcome({
  ...
  const outcome = classifyCtrDelta(delta, { regressThreshold });

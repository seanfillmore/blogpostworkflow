...
/**
 * 95% Wilson score interval for orders ÷ sessions.
 *
 * WHY AN INTERVAL AND NOT THE RATE. Every segment this module measures holds
 * single-digit orders — the clean commercial pool was 4 orders on 567 sessions
 * (2026-08-20 -> 09-15). At that size the point estimate is one order away from
 * reading 25% higher or lower, so quoting it as "the" CVR overstates what the
 * sample knows. agents/campaign-analyzer turns this interval into the range a
 * paid proposal's projected CVR must fall inside, which is exactly where false
 * precision costs money.
 *
 * WHY WILSON. The textbook normal (Wald) interval collapses to [0, 0] at zero
 * orders and goes negative near zero — both common here. Wilson stays inside
 * [0, 1], gives a real upper bound at zero orders, and is well-behaved at small n.
 * Verified against Newcombe (1998) Table I in tests/lib/commercial-cvr.test.js.
 *
 * Returns null — never a guess — when there is no denominator, or when orders
 * exceed sessions. The second happens when GA4 and Shopify attribute a purchase
 * to different landing pages (see the TWO SOURCES note in this file's header);
 * clamping it to 100% would be the most optimistic possible answer, so it is
 * treated as no evidence at all.
 *
 * @param {number} orders
 * @param {number} sessions
 * @param {number} [z] 1.96 for 95%
 * @returns {{orders:number, sessions:number, point:number, lower:number, upper:number}|null}
 */
export function wilsonInterval(orders, sessions, z = 1.96) {
  if (!Number.isFinite(orders) || !Number.isFinite(sessions)) return null;
  if (sessions <= 0 || orders < 0 || orders > sessions) return null;
  const n = sessions;
  const p = orders / n;
  const z2 = z * z;
  const denom = 1 + z2 / n;
  ...
  // Exact at the edges rather than 1e-17 off them: a lower bound that prints as
  // "0.00%" but compares greater than 0 is a bug waiting in any range check.
  const lower = orders === 0 ? 0 : Math.max(0, centre - half);
  const upper = orders === sessions ? 1 : Math.min(1, centre + half);
  return { orders, sessions, point: p, lower, upper };
}

...
/**
 * Freshness date of a derived `latest.json` report — read from its `generated_at`
 * timestamp (the date portion). Returns null if the file is missing/unparseable
 * or has no generated_at. Lets checkFreshness() cover not just raw snapshot dirs
 * but the analysis reports built from them (quick-wins, gsc-opportunity, …).
 */
export function newestReportDate(latestJsonPath) {
  if (!existsSync(latestJsonPath)) return null;
  try {
    const j = JSON.parse(readFileSync(latestJsonPath, 'utf8'));
    const ts = j.generated_at;
    if (!ts) return null;

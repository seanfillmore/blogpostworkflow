...
export async function ingestConversionEvents(requestBody, { timeoutMs = 60_000 } = {}) {
  ...
  // Google reports per-event problems without failing the request, so a 200 alone is not
  // evidence anything landed. Treat any returned error as a rejected event.
  const errors = (data.errors || []).map((e) => ({ message: e.message || JSON.stringify(e) }));
  return {
    accepted: requestBody.events.length - errors.length,
    errors,

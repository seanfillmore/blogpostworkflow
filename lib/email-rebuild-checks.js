 ...
 */
/**
 * Is there an unsubscribe merge tag at all? Either spelling counts — they are the same
 * destination, and `unsubscribeFindings` is what judges whether it is WELL FORMED.
 *
 * Exported because builders kept hand-rolling `/unsubscribe/i` against the whole body,
 * which passes on the word appearing in ordinary prose ("the unsubscribe link at the
 * bottom works immediately") while the actual tag is absent or broken. Klaviyo only
 * auto-appends a working link when it detects NO tag, so a malformed one is worse than
 * none: detection passes, the safety net never fires, and the recipient gets a dead link
 * plus raw markup as visible text.
 */
export function hasUnsubscribeTag(html) {
  return /\{%\s*unsubscribe(_link)?\s*%\}/i.test(html);
}

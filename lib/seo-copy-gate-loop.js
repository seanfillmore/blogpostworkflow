 ...
 * @param {'editorial'|'commercial'} [opts.surface]
 *        Which surface is speaking. COMMERCIAL is the default and the strict
 *        one: naming a skin condition there positions the product FOR it.
 *        Blog titles and metas pass 'editorial', so an article may be ABOUT a
 *        condition. Cure-talk blocks on either.
 ...
 * @param {Array<{check:(fields:object)=>Array<object>, constraint:(violations:Array<object>)=>string}|null>} [opts.extraChecks]
 *        Caller-specific BLOCKING checks that ride the same two-attempt budget —
 *        e.g. lib/ingredient-absence-claims.js, which needs the product's own
 *        ingredient list and so cannot live inside the generic health gate. Each
 *        check's hits join `violations`, and each supplies its OWN retry
 *        constraint: the cosmetic-compliance text would tell a model that wrote
 *        "no water" about a lotion containing water to stop naming medications.
 *        `null` entries are skipped, so a caller can pass a check that only
 *        exists when its input does.
 * @returns {Promise<GateResult>}
 */
export async function gateGeneratedCopy(generate, { extract, required = [], lengths = {}, surface, extraChecks = [] } = {}) {
  if (typeof extract !== 'function') throw new TypeError('gateGeneratedCopy: `extract` is required');
  const checks = extraChecks.filter(Boolean);
  ...
  for (let i = 0; i < 2; i++) {
    ...
    const health = checkSeoCopyFields(fields, surface ? { surface } : undefined);
    check = health;
    ...
    // Caller-specific checks block exactly like a health claim and spend the
    // same retry; each one writes its own retry constraint below.
    const extraHits = checks.map((c) => ({ c, hits: c.check(fields) || [] }));
    const extraBlocking = extraHits.flatMap((x) => x.hits);

    const missing = required.filter((f) => !String(fields[f] ?? '').trim());
    if (missing.length || extraBlocking.length) {
      check = {
        ok: false,
        blocking: [
          ...health.blocking,
    ...
    constraint = [
      seoCopyConstraint(health.blocking),
      ...extraHits.filter((x) => x.hits.length).map((x) => x.c.constraint(x.hits)),

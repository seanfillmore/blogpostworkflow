...
export function treatmentCoverage(wave = {}, trackerEntries = []) {
  ...
  for (const e of Array.isArray(trackerEntries) ? trackerEntries : []) {
    ...
    // Only a rewrite that reached the SERP treats a page. Entries before
    // 2026-09-21 wrote article.title / summary_html, which the SERP does not
    // render (lib/serp-copy.js); counting them would let a wave conclude "no
    // effect" on a treatment that never happened. Same failure direction as
    // above: under-count, which only delays a verdict.
    if (e?.serpFields !== true) continue;

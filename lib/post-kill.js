...
import { isArticleLive } from './post-kill-live.js';

export { isArticleLive };
...
export async function killPost(slug, { reason = 'killed', redirectTo = null } = {}) {
  ...
  let article = null;
  // Whether `article.published_at` is KNOWN. A null there means "draft" only
  // when Shopify said so; a null read off local meta means "never recorded".
  let publishStateKnown = false;
  if (meta?.shopify_blog_id && meta?.shopify_article_id) {
    ...
    publishStateKnown = meta.published_at != null;
    if (!publishStateKnown) {
      // THE 2026-09-21 BUG: the legacy corpus often records no published_at, so
      // a LIVE article (published 2024-04-22) read as unpublished, got no
      // redirect, and was deleted into a hard 404. Ask Shopify instead.
      try {
        const live = await findArticleByHandle(article.handle);
        if (live && String(live.id) === String(article.id)) {
          article = { ...article, published_at: live.published_at, body_html: live.body_html };
          publishStateKnown = true;
        }
      } catch { /* stays unknown — treated as live below */ }
    }
  } else {
    ...
    try {
      ...
      if (article) { summary.shopify_resolved_by = 'handle lookup'; publishStateKnown = true; }
  ...
  if (article) {
    // UNKNOWN IS LIVE. A redirect on a draft's path is harmless; a missing one
    // on a live page is a 404 for every inbound link. Same rule as
    // lib/post-lock.js: "could not tell" never counts as the safe answer.
    const isLive = isArticleLive(article.published_at, publishStateKnown);

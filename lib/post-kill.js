...
import { deleteArticle, getBlogs, getArticles } from './shopify.js';
...
export async function findArticleByHandle(handle) {
  for (const blog of await getBlogs()) {
    for (const a of await getArticles(blog.id, { limit: 250 })) {
      if (a.handle === handle) {
        return { id: a.id, blogId: blog.id, handle: a.handle, published_at: a.published_at, title: a.title };
      }
    }
  }
  return null;
...
export async function killPost(slug, { reason = 'killed' } = {}) {
  ...
  if (meta?.shopify_blog_id && meta?.shopify_article_id) {
    ...
    try {
      const found = await findArticleByHandle(slug);
      if (!found) {
        summary.warnings.push('no meta.json, and no Shopify article with this handle — nothing to delete');
      } else {
        await deleteArticle(found.blogId, found.id);
        summary.shopify_deleted = true;
        summary.shopify_resolved_by = 'handle lookup';
        // A LIVE page needs a redirect, and this function does not make one —
        // say so rather than letting a silent 404 be the outcome.
        if (found.published_at && new Date(found.published_at) <= new Date()) {
          summary.warnings.push(
            `article was LIVE (published ${found.published_at}) — killPost does NOT create a redirect; `
            + 'create one for its URL or every inbound link now 404s',
          );
          summary.was_live = true;
        }
      }
    } catch (e) {
      summary.warnings.push(`shopify handle lookup/delete failed: ${e.message}`);
    }
  }

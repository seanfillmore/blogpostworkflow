...
export function unsupportedNumbers(copy, html) {
  ...
  const out = [];
  // A number and up to three words after it: "7 Clean Picks", "5-Min Recipe".
  for (const m of String(copy || '').matchAll(/\b(\d{1,3})\b(?!\s*%)((?:[\s-]+[a-z]+){0,3})/gi)) {
    const n = Number(m[1]);
    if (m[1].length === 4) continue;
    const num = NUMBER_WORDS[n] ? `(?:${n}|${NUMBER_WORDS[n]})` : `${n}`;
    // Supported only where the body pairs the SAME number with one of the SAME
    // words (by stem, within four words). A bare digit elsewhere on the page —
    // "Recipe 7", "5–6 drops" — says nothing about "7 Picks".
    const stems = (m[2] || '').toLowerCase().split(/[\s-]+/).filter((w) => w.length >= 3)
      .map((w) => w.replace(/(?:es|s)$/, '').slice(0, 5));
    const supported = stems.length
      ? stems.some((stem) => new RegExp(`(^|[^\\d])${num}(?:[\\s-]+[a-z]+){0,4}?[\\s-]+${stem}`, 'i').test(body))
      : new RegExp(`(^|[^\\d])${num}([^\\d]|$)`, 'i').test(body);
    if (!supported && !out.includes(n)) out.push(n);
  }
  return out;
}

...
export function applyOperatorOverlay(personasData, overlay) {
  ...
  for (const [personaId, angleList] of byPersona.entries()) {
    if (!known.has(personaId)) {
      const angleIds = angleList.map(a => a?.id ?? '(no id)').join(', ');
      throw new Error(
        `operator-angles: angle(s) [${angleIds}] in data/context/operator-angles.json name persona ` +
        `"${personaId}", which is not in personas.json (known personas: ${[...known].join(', ') || 'none'}). ` +
        `This usually means the monthly voice-of-customer run renumbered the personas. Fix: in ` +
        `data/context/operator-angles.json, either update the "personaId" field on angle(s) [${angleIds}] ` +
        `to a current persona id, or remove the entry entirely if it no longer applies — leaving it as-is ` +
        `will keep failing this way.`
      );
    }
  }

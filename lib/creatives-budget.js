...
/** The repo's `.env` — the only place the server's ceiling is actually written down. */
export const DEFAULT_ENV_FILE = join(ROOT, '.env');
 ...
 * why. The server sets CREATIVES_BUDGET_BYTES to 4 GiB in `.env`, which
 * configuredBudgetBytes() reads directly — cron never sources it.
 */
...
/** `KEY=value` lines of a .env file, or {} if it is missing or unreadable. */
...
/**
 * The ceiling AS CONFIGURED — process.env first, then `.env`, then the local default.
 *
 * The `.env` fallback is here, in the resolver, rather than as hydration at each entry
 * point, because entry-point hydration is what already failed: only the dashboard called
 * it, so the two paths that most need the server's 4 GiB ceiling never saw it. The weekly
 * cron line is `cd /root/seo-claude && /usr/bin/node scripts/creatives-budget.mjs
 * --apply` — cron does not source `.env` — and a hand-run agent gets nothing either,
 * because ad-studio's loadEnv() parses `.env` into a LOCAL object on purpose and never
 * populates process.env. One resolver covers every caller, present and future, and it
 * mutates no global state, so that deliberate stance is untouched.
 *
 * process.env still wins, so the dashboard-spawned agent's inherited value and any
 * one-off override both behave as before.
 */
export function configuredBudgetBytes({ env = process.env, envFile = DEFAULT_ENV_FILE } = {}) {
  const direct = Number(env?.CREATIVES_BUDGET_BYTES);
  if (Number.isFinite(direct) && direct > 0) return direct;
  return resolveBudgetBytes(readEnvFile(envFile));
}
 ...
 * @param {number} [args.budgetBytes]  defaults to the CONFIGURED ceiling (process.env, then .env)
 * @param {string} [args.envFile]      where that fallback reads from; a test seam only
...
export function planPurge({
  creativesDir,
  // Declared before budgetBytes on purpose: object-destructuring defaults are evaluated
  // in order, so budgetBytes' default can read it.
  envFile = DEFAULT_ENV_FILE,
  budgetBytes = configuredBudgetBytes({ envFile }),
  keepPaths = [],
  now = Date.now(),

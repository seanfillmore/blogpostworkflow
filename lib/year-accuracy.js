...
// An ISO-8601 date: the year is followed by -MM-DD. "2025-06-25T11:00:07-06:00".
// A publication timestamp is a FACT ABOUT THE PAST — bumping it would falsify
// when the article was published, which is worse than leaving a stale marker.
// `(?!\d)` rather than `\b`: in "2025-06-25T11:00:07" the char after the day is
// `T`, a word character, so `\b` never matches and the whole guard is dead.
const ISO_DATE_AFTER = /^-\d{2}-\d{2}(?!\d)/;
// A month name immediately before the year makes it a DATE, not an edition marker:
// "December 2025", "in Dec 2025", "(December 2025)".
//
// THIS ONE HAS ALREADY CAUSED HARM, which is why it is worth its trade-off.
// `bumpStaleYears` shares this predicate, and on `best-clean-body-lotion-2025`
// and `best-non-toxic-body-lotion-2025` it rewrote the References entry "FDA
// Report on the Use of PFAS in Cosmetic Products (December 2025)" to "(December
// 2026)" — a report dated in the FUTURE, contradicting the body of the same page,
// which still read December 2025. The FDA published it on 2025-12-29. A citation
// date is a fact about the past; bumping one manufactures a false citation.
//
// The trade-off, stated plainly: an edition marker genuinely written as "Updated
// December 2025" will no longer be bumped. That is the right way to lose — a
// stale "Updated" line is a cosmetic miss, a fabricated source date is not.
const MONTH_PREFIX = /\b(jan(?:uary)?|feb(?:ruary)?|mar(?:ch)?|apr(?:il)?|may|jun(?:e)?|jul(?:y)?|aug(?:ust)?|sep(?:t|tember)?|oct(?:ober)?|nov(?:ember)?|dec(?:ember)?)\.?\s+$/i;
...
/**
 * True when the year at `index` sits inside a URL, path or slug rather than in
 * prose — i.e. the whitespace-delimited token containing it holds a `/`.
 *
 * A HANDLE IS A URL, NOT A DESCRIPTION. Renaming a slug to bump its year breaks
 * a live ranking page and gains nothing, which is the same rule CLAUDE.md states
 * for the antiperspirant handles. So a year here is never stale, whatever it
 * says — and this is checked STRUCTURALLY rather than by asking a prompt not to
 * do it, because the prompt was already asking (`agents/editor`'s rule 4 lists
 * four kinds of attribute it must ignore) and the deterministic check that
 * overrides the LLM was reading them anyway.
 *
 * Whitespace-delimited is deliberate: it needs no URL parser, it cannot be
 * defeated by a relative path, and prose that merely mentions a year near a
 * slash ("the 2024 / 2025 season") keeps whitespace around the slash and is
 * unaffected.
 *
 * @param {string} text
 * @param {number} index start offset of the 4-digit year within `text`
 */
export function isInUrlOrSlug(text, index) {
  let start = index;
 ...
 * (historical fact, citation, range, founding date, copyright, ISO date) or is
 * part of a URL/slug — rather than a stale current-year marker to be bumped.
 *

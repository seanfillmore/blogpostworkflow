...
/**
 * The keyword-index clusters that make up "skin" as this artifact uses the word: the
 * lotion + soap product line voice-of-customer's SKIN_CLUSTER_HANDLES researches
 * (coconut-lotion, body-lotion-1, coconut-moisturizer, coconut-soap,
 * organic-foaming-hand-soap) — never deodorant, toothpaste, lip balm or hair, which
 * are separate clusters with their own product lines.
 *
 ...
 * skin-relevant and would otherwise be dropped just for not saying "lotion" or
 * "soap" outright. This does let a handful of off-topic generic-ingredient queries
 * (e.g. a cooking question) through mislabeled — an accepted, pre-existing
 * imprecision of assignCluster's own bucket (see its docstring), not something this
 * filter introduces.
 *
 ...
 * is excluded, never 'coconut oil'/skin. That specific failure mode (~a third of a
 * real leak report's top entries were oral care) is the reason this filter exists:
 * the Prime Directive names the toothpaste cluster as ≈268 clicks / $0 revenue, so
 * mining it on a paid budget and then stamping the result `cluster: "skin"` would be
 * both a wasted spend and a mislabeled artifact.
 */
export const SKIN_LEAK_CLUSTERS = ['lotion', 'soap', 'coconut oil'];
const SKIN_LEAK_CLUSTER_SET = new Set(SKIN_LEAK_CLUSTERS);

/**
 * Filter GSC impression-leak queries down to the skin cluster, before they are ever
 * handed to deriveSeeds. Leaks are unfiltered site-wide GSC data — unlike persona
 * objections, which are already skin-scoped by construction (personas.json is
 * voice-of-customer's skin-cluster research) — so this is the one place the leak
 * origin needs an explicit cluster check.
 *

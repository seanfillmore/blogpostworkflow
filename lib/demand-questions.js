...
/**
 * The keyword-index clusters GSC impression-leak queries are mined from, before a
 * leak ever consumes a paid seed. assignCluster (lib/keyword-index/cluster.js) can
 * return: brand, coconut oil, deodorant, hair, lip balm, lotion, soap, toothpaste,
 * unclustered. This set is every one of Real Skin Care's actual product lines —
 * deodorant, lip balm, lotion, soap, plus the generic coconut-oil ingredient bucket
 * — with two clusters deliberately excluded, for different reasons:
 *
 * - 'toothpaste' — the Prime Directive names this cluster ≈268 clicks and $0 revenue
 *   (CLAUDE.md). Mining it would spend a paid DataForSEO seed harvesting demand for
 *   a product line that already gets traffic and converts none of it into orders.
 * - 'hair' — Real Skin Care sells no hair products (CLAUDE.md's Brand Context
 *   table). A "hair" leak names demand for something not for sale: there is no PDP
 *   or collection it could ever point to, so mining it can never convert.
 *
 * 'brand' and 'unclustered' stay excluded too, unchanged from the original filter:
 * branded/navigational queries and anything assignCluster can't place don't belong
 * in a product-demand mining budget either.
 *
 * Earlier revisions of this set were 'skin'-only (lotion + soap + coconut oil),
 * matching voice-of-customer's SKIN_CLUSTER_HANDLES (lotions and soaps). The owner
 * widened it to also include 'deodorant' (Real Skin Care's first-listed product
 * line) and 'lip balm' (another named product line) — see the "cluster: 'skin'"
 * label note on filterLeaksToSkinCluster below, since that widening makes the
 * artifact's own `cluster: "skin"` field arguably inaccurate even though this
 * constant and function keep their original names.
 *
 * 'coconut oil' (the generic-ingredient bucket assignCluster falls back to when a
 * query names the ingredient but no specific product) is included deliberately: a
 * plain query like "coconut oil acne" or "coconut oil for dry skin" is genuinely
 * relevant and would otherwise be dropped just for not saying "lotion", "soap",
 * "deodorant" or "lip balm" outright. This does let a handful of off-topic
 * generic-ingredient queries (e.g. a cooking question) through mislabeled — an
 * accepted, pre-existing imprecision of assignCluster's own bucket (see its
 * docstring), not something this filter introduces.
 *
 * WHY assignCluster AND NOT voice-of-customer's SKIN_CLUSTER_HANDLES/filterSkinCluster:
 * those operate on REVIEW records carrying a `handle` (a specific product a review
 * was left on) — there is no such thing for a free-text GSC search query, so they
 * do not fit this input shape. assignCluster IS built for free-text keyword
 * classification and is already relied on elsewhere in the fleet
 * (agents/keyword-index-builder). Its ordered, first-match-wins rules are exactly
 * what this needs: the toothpaste rule (matching "toothpaste", "teeth", "tooth",
 * "fluoride", "sls", "enamel", "cavit(y)", "oral care", "mouthwash") is checked
 * BEFORE the generic 'coconut oil' rule, so an oral-care query that happens to
 * mention coconut oil — e.g. "coconut oil as toothpaste", "are coconut oil
 * toothpastes safe for sensitive teeth?" — correctly classifies as 'toothpaste' and
 * is excluded, never 'coconut oil'. That specific failure mode (~a third of a
 * real leak report's top entries were oral care) is the reason this filter exists.
 */
export const SKIN_LEAK_CLUSTERS = ['lotion', 'soap', 'coconut oil', 'deodorant', 'lip balm'];
const SKIN_LEAK_CLUSTER_SET = new Set(SKIN_LEAK_CLUSTERS);

/**
 * Filter GSC impression-leak queries down to SKIN_LEAK_CLUSTERS, before they are
 * ever handed to deriveSeeds. Leaks are unfiltered site-wide GSC data — unlike
 * persona objections, which are already skin-scoped by construction (personas.json
 * is voice-of-customer's lotion+soap skin-cluster research) — so this is the one
 * place the leak origin needs an explicit cluster check.
 *
 * NOTE on the name: this function (and SKIN_LEAK_CLUSTERS above) kept its original
 * "skin" name when the owner widened the set to add 'deodorant' and 'lip balm' —
 * neither of which voice-of-customer would call "skin". The name is now narrower
 * than what it actually filters to. See the widening note on SKIN_LEAK_CLUSTERS
 * for the full reasoning and the open question it raises for the artifact's own
 * `cluster: "skin"` field (agents/demand-miner/index.js).
 *
 * A non-array input (null/undefined — "no leak feed available") passes straight
 * through unchanged, preserving deriveSeeds' existing missing-vs-empty distinction:
 * `null` means "source unavailable" (partial), `[]` means "source available, nothing
 * survived the filter" (also partial, once deriveSeeds sees no leak-origin seeds) —
 * both degrade the same way, neither throws.
 *
 * A thin wrapper around filterLeaksToSkinClusterDetailed below — see that function
 * for why a second entry point exists instead of changing this signature.
 */
export function filterLeaksToSkinCluster(leaks) {
  return filterLeaksToSkinClusterDetailed(leaks).survivors;
}

/**
 * Same predicate as filterLeaksToSkinCluster, but reports what it dropped instead of
 * discarding it. Added to answer demand-miner's first open question — how much
 * recall the skin-cluster filter costs, and whether what it drops is genuinely
 * on-topic top-of-funnel phrasing (e.g. "why is my skin so oily", "how to get rid of
 * dark spots") that assignCluster simply can't place. Reporting that requires the
 * dropped rows themselves, not just a smaller survivor array, so this is a pure
 * sibling function rather than a change to filterLeaksToSkinCluster's existing
 * signature — its one caller (agents/demand-miner/index.js) and the tests pinning it
 * only expect a survivor array back.
 *
 * A non-array input mirrors filterLeaksToSkinCluster's pass-through: `survivors` is
 * the input unchanged, `dropped` is `[]`, `total` is `0` — "no leak feed available"
 * has nothing to report, not zero-out-of-zero.
 */
export function filterLeaksToSkinClusterDetailed(leaks) {
  if (!Array.isArray(leaks)) return { survivors: leaks, dropped: [], total: 0 };
  const survivors = [];
  const dropped = [];
  for (const l of leaks) {
    (SKIN_LEAK_CLUSTER_SET.has(assignCluster(l?.query)) ? survivors : dropped).push(l);
  }
  return { survivors, dropped, total: leaks.length };
}
 ...
 *
 * `clusters` is an array (sourced from SKIN_LEAK_CLUSTERS by the caller), not the single
 * `cluster: "skin"` string this used to render. That single word became false once
 * SKIN_LEAK_CLUSTERS grew beyond lotion+soap to also include deodorant and lip balm — see
 * SKIN_LEAK_CLUSTERS' docstring. The array is still only the LEAK-origin ceiling, though:
 * persona-objection seeds are scoped by voice-of-customer's SKIN_CLUSTER_HANDLES
 * (lotion+soap only, unaffected by this artifact's cluster list) regardless of what
 * `clusters` says, so the header note below tells a reader to check each entry's own
 * `seed_origin` rather than assume every question spans the full `clusters` list.
 */
export function renderDemandQuestionsMarkdown({ questions = [], generatedAt, clusters = [], seedCount, partial } = {}) {
  const lines = [
    '# Demand questions',
    '',
    ...
    'question. The Clusters line above scopes GSC-leak seeds only (seed_origin:',
    '"gsc_leak") — persona-objection seeds (seed_origin: "persona_objection") are always',
    'lotion + soap, per voice-of-customer\'s SKIN_CLUSTER_HANDLES, regardless of what',
    'Clusters lists. Check each entry\'s own seed_origin, not this header, to know which',
    'scope actually produced it.',
    '',
  ];

 ...
 */
/**
 * Index active signals by keyword/slug and by cluster, returning the matcher
 * `computePlan` scores with, plus the weak-signal `suggestions` list.
 *
 * Exported so the ATTRIBUTION LEDGER can attribute items that are already in
 * production. `computePlan`'s `scored` array holds only backlog ideas (the agent
 * builds `backlog` from `pending` items with no publish_date), so anything already
 * briefed/written/published is absent from it — the first version of the production
 * ledger read `plan.scored` and consequently logged nothing at all on a real run.
 * Sharing this factory keeps one copy of the matching rules rather than two that drift.
 */
export function buildContributionIndex(signals, cfg) {
  const byKey = new Map();
  ...
  return { contributionsFor, suggestions };
}
...
export function computePlan(inputs) {
  ...
  const { contributionsFor, suggestions } = buildContributionIndex(signals, cfg);
  ...
  for (const sig of signals || []) {
    ...
    const idea = {
      ...
      // Carried onto the injected idea, and from there onto the calendar item,
      // so the demotion survives the run that created it — otherwise a flagged
      // idea is demoted once and sorts as clean on every morning after.
      possible_duplicate: Boolean(sig.possibleDuplicate),
      duplicate_of: sig.duplicateOf ?? null,
  ...
  // DEMOTE, NEVER DROP. An idea an existing page may already rank for
  // (`possible_duplicate`, set upstream by lib/ranked-coverage.js and carried
  // through agents/gsc-opportunity) sorts below every clean idea, whatever its
  // score. This agent RANKS work and ordering destroys nothing, so a demotion is
  // the strongest thing the flag is allowed to do here — the
  // `lib/ctr-opportunity.js` doctrine ("it DEMOTES and never drops"), and the
  // same band `sortUnmapped` applies one step upstream.
  //
  // A `rush` override still wins: that is an explicit human instruction, and a
  // measured heuristic does not get to overrule one.
  const duplicateBand = (i) => (i.possible_duplicate ? 1 : 0);
  const rank = (a, b) => {
    const ar = a.status_override === 'rush' ? 1 : 0;
    const br = b.status_override === 'rush' ? 1 : 0;
    if (ar !== br) return br - ar;            // rush first
    const ad = duplicateBand(a) - duplicateBand(b);
    if (ad !== 0) return ad;                  // then clean ideas before flagged ones

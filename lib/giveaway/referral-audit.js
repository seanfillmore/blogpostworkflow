...
export function classifyReferrals(profiles = [], { mechanism = CONFIRM_MECHANISMS.DOUBLE_OPT_IN } = {}) {
  ...
  for (const p of profiles) {
    ...
    // THE NEAR-MISS LOOKUP RUNS BEFORE ANY BLOCKER IS RETURNED, and that ordering is
    // the whole point of computing it. It used to sit last, after the
    // `referee_unconfirmed` return, so a pair was only ever examined for a typo once
    // the REFEREE had confirmed — two unrelated conditions, and the wrong one gating
    // the other. Measured on the 2026-08-24 08:40 production run: 17 of 26 pairs
    // short-circuited there, `referrer_near_miss` reported 0, and the operator was
    // reconciling misspellings BY HAND against a report built to surface them. Among
    // the hidden 17 was a distance-1 pair (`carlacythurston@yahoo.como` naming
    // `carlacythurston@yahoo.com`). A typo is a fact about the typed address; whether
    // the referee has confirmed cannot change it, and confirmation is a state that
    // moves on its own every night.
    //
    // The suggestion is attached to whatever status is returned, so the blocker still
    // reads true — `referee_unconfirmed` with a suggestion means BOTH are true, which
    // is exactly the situation. Still recorded and never acted on: §5 makes the typed
    // value the identifier, so this is evidence for a human, never a rewrite.
    ...
    if (row.suggestion) {
      rows.push({
        ...
        reason: `the typed address is ${row.suggestion.distance} edit(s) from confirmed entrant ${row.suggestion.email}, which §5 does not permit us to correct`,
      });
      continue;
    }

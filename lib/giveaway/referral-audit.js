...
import { confirmedEver } from './reconcile.js';
 ...
 */
export function classifyReferrals(profiles = []) {
  ...
  for (const p of profiles) {
    ...
    byEmail.set(email, p);
    if (confirmedEver(p)) confirmedEmails.push(email);
  }
  ...
  for (const p of profiles) {
    ...
    const row = {
      referee, namedRaw, namedReferrer: null, status: null, suggestion: null, notify: null, reason: '',
    };
    ...
    if (looksSamePerson(named, referee)) {
      rows.push({ ...row, status: 'self_referral_suspected', reason: 'the two addresses appear to resolve to the same person — §6 requires a human determination' });
      continue;
    }
    if (!confirmedEver(p)) {

...
export function summarizeEntrants(profiles) {
  ...
  // `referrals` and `entrantsWithReferrals` count CREDITED referrals only — a rung
  // pays once both parties confirm. `referralsNamed` counts everyone who typed an
  // address into the optional referrer field, credited or not, and it exists because
  // reporting only the credited figure reads as "nobody is referring anyone" when the
  // truth is "people referred and the credit has not landed yet". On 2026-08-24 that
  // was 41 named against 0 credited, and the 0 was quoted to the operator as zero
  // participation — a reporting artifact presented as a CTA failure. A referral is
  // pending, not failed: reconcile.js re-evaluates nightly and pays the moment the
  // other half confirms. NEVER show the credited count without this denominator.
  const ladder = {
    confirmed: 0, survey: 0, referrals: 0, instagram: 0, upload: 0,
    entrantsWithReferrals: 0, referralsNamed: 0,
  };
  let entriesTotal = 0;
  ...
  for (const profile of profiles) {
    ...
    // Read off the raw field the entry form writes, NOT off gv_breakdown: the
    // breakdown only ever records what has been credited, so it cannot answer
    // "did anyone try". A blank or whitespace-only value is not an attempt.
    if (String(props.gv_referred_by ?? '').trim()) ladder.referralsNamed += 1;

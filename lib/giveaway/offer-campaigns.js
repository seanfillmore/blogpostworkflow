...
import { hasUnsubscribeTag, unsubscribeFindings } from '../email-rebuild-checks.js';
...
export function checkOfferEmail(html, { variantId = BAR_VARIANT_ID } = {}) {
  ...
  // Two questions, not one. Presence is CAN-SPAM; well-formedness is what decides
  // whether the link the recipient sees actually works. `{% unsubscribe %}` expands to a
  // complete <a> element, so inside an href it renders href="<a class=" — a dead link,
  // with the rest of the footer leaking out as visible text. Klaviyo auto-appends a
  // working link only when it finds NO tag, so a broken tag defeats its own safety net.
  if (!hasUnsubscribeTag(html)) problems.push('no unsubscribe merge tag — required on a promotional send');

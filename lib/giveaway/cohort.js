...
export function entryValue(cohort, { segment = 'new' } = {}) {
  ...
  for (const days of [...WINDOWS].reverse()) {
    ...
    if (w?.matured) {
      return { segment, basis: `${days}d`, value: w.revenuePerEntrant, matured: w.matured, windowMatured: true };
    }
  }
  // THE FALLBACK IS NOT A MEASUREMENT, and `matured` here does not mean what it means
  // above. In the loop it is the count of entrants whose window has CLOSED; here it is
  // simply how many entrants exist, none of whom have had time to buy. The name collision
  // is what fooled evaluateSpendGate on 2026-08-20: two entrants, hours old, zero revenue,
  // and the gate read "a new entrant is measurably worth $0" rather than "nothing is
  // measurable yet" — then told the operator to fix creative on day one of a campaign that
  // was performing fine. windowMatured is the unambiguous signal; read that, not `matured`.
  return {
    segment,

 ...
 */
export const confirmedEver = (profile) => {
...
export function planEntryUpdates(profiles, { now = new Date().toISOString() } = {}) {
  ...
  //
  // Callers must therefore pass submitted-but-unlisted profiles too (Klaviyo
  // only adds a profile to the list once double opt-in completes, so the list
  // alone is the confirmed set). See reconcile-referrals.mjs.
  ...
  for (const [email, p] of byEmail) {
    if (confirmedEver(p)) confirmedEmails.add(email);
  }

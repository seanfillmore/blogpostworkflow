 ...
 */
/**
 * Every reminder actually sent, oldest first. Measured from Klaviyo's
 * campaign-values report, not estimated.
 *
 * IT IS A LIST BECAUSE ONE SEND IS NOT A RATE. The first version of this module
 * hardcoded reminder #1 and projected every future send from it. Reminder #2
 * then came in at less than HALF the confirm rate and well above the complaint
 * rate, so a third send sized from #1 alone would have printed "below the 0.3%
 * enforcement line" while the most recent evidence said the opposite — the guard
 * under-warning at exactly the moment it mattered.
 */
export const REMINDER_SENDS = Object.freeze([
  Object.freeze({
    label: '#1',
    campaignId: '01M0RZM53084R8VEM8A2MS63PZ',
    sentAt: '2026-08-25T14:00:00Z',
    recipients: 489, delivered: 487, opensUnique: 133, clicksUnique: 33,
    unsubscribes: 4, spamComplaints: 1, bounced: 2,
  }),
  Object.freeze({
    label: '#2',
    campaignId: '01M1SNWVSB1ZMF6V8K2VBW2BTY',
    sentAt: '2026-09-05T21:59:10Z',
    recipients: 901, delivered: 896, opensUnique: 109, clicksUnique: 22,
    unsubscribes: 6, spamComplaints: 3, bounced: 5,
  }),
]);

/** Back-compatible alias: the send whose instant defines "already reminded". */
export const FIRST_REMINDER = REMINDER_SENDS[0];

/** The most recent send — the best estimate of how the REMAINING population behaves. */
export const LATEST_REMINDER = REMINDER_SENDS[REMINDER_SENDS.length - 1];
...
export function projectReminderOutcome(recipientCount, sends = REMINDER_SENDS) {
  const latest = sends[sends.length - 1];

  // YIELD comes from the MOST RECENT send. Each reminder mails a colder cohort
  // than the last — everyone who was going to confirm easily already has — so
  // the newest number is the only one describing who is left. Measured: #1
  // confirmed 6.78% of delivered, #2 confirmed 2.46%, a 2.8x decay in eleven
  // days. Averaging them would predict a yield neither send produced.
  const confirmRate = latest.clicksUnique / latest.delivered;
  const unsubRate = latest.unsubscribes / latest.delivered;

  // COMPLAINT RATE comes from the WORST send ever observed, and the asymmetry is
  // the reason. Over-estimating yield costs a disappointing report; under-
  // estimating complaints costs the sending domain's reputation, which is shared
  // with every flow, deadline campaign and post-purchase email this store sends
  // and is slow and expensive to repair. When the two disagree, take the number
  // that makes you send less.
  const spamRate = Math.max(...sends.map((s) => s.spamComplaints / s.delivered));
  const worst = sends.find((s) => s.spamComplaints / s.delivered === spamRate);

  return {
    recipients: recipientCount,
    confirmRate,
    spamRate,
    basis: { yieldFrom: latest.label, complaintsFrom: worst.label, sends: sends.length },

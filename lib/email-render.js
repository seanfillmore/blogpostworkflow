...
// CAN-SPAM footer address. Single source of truth is brand-kit.json — it previously
// lived as a duplicated literal in four files and changed once already.
const ADDRESS = `Real Skin Care · ${KIT.postal_address}`;
...
/**
 * Products are made in the USA. They are NOT made in Blum, Texas — that address is
 * distribution, not manufacturing, and it is past tense besides.
 *
 * A redesign wrote "made in small batches in Blum, Texas" into Winback 03 and it reached
 * production before anyone caught it. The postal address legitimately names the same
 * town, so the check has to distinguish an origin claim from the CAN-SPAM footer rather
 * than ban the word.
 *
 * Approved phrasing: "made in the USA" / "handmade in the USA".
 */
...
function assertNoOriginClaim(html) {
  // Strip the footer address first — it names Blum legitimately.
  const withoutAddress = html.replace(ADDRESS, '');
  const hit = withoutAddress.match(ORIGIN_CLAIM);
  if (hit) {
    throw new Error(
      `origin claim names a location: "${hit[0].trim()}" — products are made in the USA, `
      + 'not in Blum, Texas (that address is distribution). Use "made in the USA".',
    );
  }
}

/**
 * The free-shipping threshold appears in six emails' copy. It moved from $50 to $45 on
 * 2026-07-31 and had to be corrected by hand in a live template, so a superseded figure
 * now fails the build.
 *
 * Matched only where a number reads as a threshold — "over $45", "starts at $45" — so
 * product prices like "$46.80" and "$8" are untouched.
 */
const THRESHOLD_PHRASE = /\b(?:over|above|starts at|from|of)\s*\$(\d+)(?!\.\d)/gi;

// A threshold that lost its currency symbol — "orders over 45 ship free" — is what a
// dropped escape in the interpolation produced, and the stale-value check above could not
// see it because it requires a leading "$". Anchored on shipping wording so ordinary prose
// ("over 20 years ago", "1–2 weeks") is untouched.
const BARE_THRESHOLD = /\b(?:over|above|starts at|from)\s+(\d+)\s+(?=ship|shipping)/gi;

function assertShippingThreshold(html) {
  for (const m of html.matchAll(THRESHOLD_PHRASE)) {
    if (Number(m[1]) !== KIT.free_shipping_threshold) {
      throw new Error(
        `stale free shipping threshold "${m[0].trim()}" — the current one is `
        + `$${KIT.free_shipping_threshold} (brand-kit.json free_shipping_threshold)`,
      );
    }
  }
  const bare = html.match(BARE_THRESHOLD);
  if (bare) {
    throw new Error(
      `shipping threshold is missing its currency symbol: "${bare[0].trim()}" — `
      + 'interpolate as $${SHIP}, not ${SHIP}',
    );
  }
}
...
`;
  ...
  assertPalette(html);
  assertNoOriginClaim(html);
  assertShippingThreshold(html);
  return html;
}

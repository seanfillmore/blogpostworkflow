 ...
 * this replaces.
 *
 * **The word list is deliberately CASE-SENSITIVE — lower case only.** These
 * titles are written in title case, where a trailing LOWER-case connective is
 * dangling ("Best Hypoallergenic Deodorant for") while a CAPITALISED one is
 * usually a real particle of the phrase ("What to Look For"). Matching
 * case-insensitively strips the second kind too, turning
 * "Best Organic Toothpaste: What to Look For" into "…What to Look" — shorter,
 * and worse. The separator class stays case-irrelevant because punctuation has
 * no case.
 */
// Words that dangle at the END of a title in ANY case. This list is separate
// from DANGLING_TAIL because that one is deliberately lower-case-only, to keep
// a capitalised particle like "What to Look For". But a title-cased "Your",
// "You" or "The" is dangling however it is capitalised — "The Ultimate Guide To
// Moisturizing Your" and "…Clean Skin Guide You" are both real output.
//
// Kept deliberately tight. `for` is NOT here (it would break "What to Look For")
// and neither is `why` ("Natural Deodorant: Here's Why" is a real ending). An
// entry belongs here only when no sensible title ends on it.
const ALWAYS_DANGLING = /\s+(?:that|actually|really|very|and|or|but|so|than|then|which|when|while|into|onto|its|their|his|her|your|you|the|a|an|this|these|those|my|our|we|us|is|are|was|were|be|been|how)$/iu;

const DANGLING_TAIL = /(?:[\s|\u2013\u2014\u2010-\u2015:;,.&\-]+|\s+(?:and|or|for|the|to|with|of|in|on|at|a|an|by|from|vs|your|our|is|are|what|how|why|that|it))+$/u;

/**
 * Drop an opening bracket the cut never closed, along with whatever followed it.
 * "Best Aluminum Free Deodorant in 2026 (That" is a word boundary with no
 * dangling connective and still obviously broken.
 */
function closeBrackets(s) {
  let out = s;
  for (const [open, close] of [['(', ')'], ['[', ']'], ['{', '}']]) {
    const o = out.lastIndexOf(open);
    if (o !== -1 && out.indexOf(close, o) === -1) out = out.slice(0, o);
  }
  return out;
}

/** Trim separators, punctuation, dangling connectives and unclosed brackets off the end. */
/**
 * Drop a trailing clause the cut ORPHANED. "Best Soap for Tattoos – Natural" is
 * the surviving stub of "…– Natural, Fragrance-Free & Gentle", and once the
 * theme appends its own " – Real Skin Care" it reads as two dangling dashes.
 * Only short stubs go: a substantial trailing clause is real copy, not debris.
 * "Best Clean Lotion – Non-Toxic, Organic" keeps its tail; "…| Natural" does not.
 */
const ORPHAN_STUB_MAX = 8;
const SEP = '|\\u2013\\u2014\\u2010-\\u2015:';
const ORPHAN_RE = new RegExp(`^(.*\\S)\\s*[${SEP}]\\s*([^${SEP}]{1,${ORPHAN_STUB_MAX}})$`, 'u');
function dropOrphanedClause(s) {
  const match = ORPHAN_RE.exec(s);
  if (!match) return s;
  // Never strip back to nothing, and never strip a stub that is itself most of
  // the title — that would be a rewrite, not a tidy.
  const head = match[1].trim();
  if (head.length < 12) return s;
  return head;
}

/** Trim separators, punctuation, dangling connectives, unclosed brackets and orphaned clauses. */
/**
 * A question stub the cut orphaned after a clause break: "Dangerous? What",
 * "Benefits, Uses & What", "What It Is & How It", "Does It Work & Is It".
 * Title case, so DANGLING_TAIL (lower-case by design) never saw them; found on
 * 11 live title_tags on 2026-09-21, including "SLS Free Toothpaste: Top Picks &
 * What" on the site's biggest page. Only stripped after `&`, `and`, a comma or a
 * `?`, so a complete phrase such as "What to Look For" is never touched.
 */
const QUESTION_STUB = /(?:\s*[&,]|\s+and|(?<=\?))\s+(?:what|why|how|is|does|do|can)(?:\s+(?:it|you|they))?$/iu;

function tidyTail(s) {
  let out = String(s ?? '').trim();
  for (let i = 0; i < 6; i++) {
    let next = closeBrackets(out.replace(QUESTION_STUB, '').replace(DANGLING_TAIL, '')).trim();
    next = next.replace(ALWAYS_DANGLING, '').trim();
    next = dropOrphanedClause(next).trim();
    if (next === out) break;
    out = next;
  }
  return out;
...
export function shortenToRenderedLimit(title, { max = LENGTH_LIMITS.title.max, shopName = SHOP_NAME } = {}) {
  ...
  if (chars.length > budget) {
    ...
    // Prefer a CLAUSE boundary over a bare word boundary. Cutting
    // "Best Clean Body Lotion: Soft Skin, Zero Toxins" at the last space leaves
    // "…Soft Skin, Zero"; cutting at the comma leaves "…Soft Skin", which reads
    // like a title somebody wrote. Only accept it if it keeps enough of the
    // title to still carry the keyword.
    // Two floors, and the gap between them is the whole point. A clause cut
    // reads better but throws away everything after it, so it is only worth
    // taking LATE — at 0.55 of the budget it discarded half of
    // "Best Organic Toothpaste: What to Look For & Why It", leaving
    // "Best Organic Toothpaste" and 20 characters of budget unspent. At 0.70 the
    // same title falls through to the word boundary and keeps
    // "…: What to Look For", while "…Soft Skin, Zero Toxins" still cuts at its
    // comma. Prefer the clause only when it costs little.
    const clauseFloor = Math.floor(budget * 0.70);
    const wordFloor = Math.floor(budget * 0.55);
    const lastClause = Math.max(
      cut.lastIndexOf(','), cut.lastIndexOf(':'), cut.lastIndexOf(';'),
      cut.lastIndexOf('|'), cut.lastIndexOf('\u2013'), cut.lastIndexOf('\u2014'),
    );
    const lastSpace = cut.lastIndexOf(' ');
    if (lastClause >= clauseFloor) cut = cut.slice(0, lastClause);
    else if (lastSpace >= wordFloor) cut = cut.slice(0, lastSpace);
    prose = tidyTail(cut);
  }
  return prose;
}

...
//
// ── TITLES ARE DELIBERATELY NOT GATED HERE, AND THE REASON IS MEASURED ─────────
//
// The obvious companion rule — "title at most 60" — is WRONG on this site, and
// shipping it would have certified titles that truncate. **The theme appends
// ` – Real Skin Care` (17 characters) to every rendered `<title>`**, verified by
// fetching live pages on 2026-09-06:
//
...
//
// So the effective budget for a generated title is ~43 characters, while every
// prompt in the fleet asks for **50–60** — meaning the instruction itself
// guarantees a truncated title. That is why Ahrefs reports `Title too long` on
// 66 URLs while only ~10% of `title_tag` values exceed 60 on their own
// (n=122, median 40, p90 60, max 65).
//
// Fixing it means changing the stated target in five agents' prompts AND making
// the limit suffix-aware, which is a different change with its own blast radius
// — so `title` is absent from LENGTH_LIMITS rather than present with a number
// that would be wrong in one direction or fire on every run in the other. It is
// recorded here so the next reader does not "complete" this module by adding a
// flat 60 and making things worse.
//
...
export function checkCopyLength(fields, kinds = {}) {
  ...
  for (const [field, kind] of Object.entries(kinds)) {
    ...
    // Count by CODE POINT, not UTF-16 unit. `value.length` counts an emoji or
    // any astral character as 2, which would over-report the length of copy
    // that renders as one glyph. [...value] iterates code points.
    const length = [...value].length;
    if (length > limit.max) {
      overlong.push({ field, kind, length, max: limit.max, over: length - limit.max });
    }
  }
...
export function lengthConstraint(overlong) {
  ...
  const lines = overlong.map(({ field, kind, length, max, over }) => {
...
export const SEO_COPY_LENGTH_RULE = [
  ...
  'Anything longer is truncated mid-sentence in Google results and the cut-off text is wasted. Count the characters before returning.',
].join('\n');

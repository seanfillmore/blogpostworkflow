 ...
 */
/**
 * Slug form, for comparing an `exact` rejection against a keyword written with
 * different punctuation. "sls free" and "sls-free" are the same keyword; calendar
 * markdown and a dashboard form disagree about which one they hand us.
 */
function rejSlug(s) {
...
/**
 * THE matching rule. One rule, because there were SEVEN and they disagreed.
 *
 * `content-strategist`, `pipeline-scheduler`, `unmapped-query-promoter`,
 * `gsc-opportunity`, `quick-win-targeter`, the dashboard's `data-parsers` and
 * `lib/brief-triage.js` each hand-rolled this, with three different meanings for
 * `matchType: 'exact'` — raw string equality, slug-normalized equality, and (in
 * brief-triage, which was handed a flattened array of STRINGS) exact-only with
 * `matchType` ignored entirely.
 *
 * The semantics here are the union of the two that agreed on intent:
 *   exact              → equal as raw strings OR as slugs
 *   anything else      → substring, which is what an entry with no matchType has
 *                        always meant to `content-strategist`, the agent that
 *                        WRITES most of them
 *
 * Accepts a bare string as an entry, because `scripts/triage-orphan-briefs.mjs`
 * flattened the list that way and a shared rule that rejected its input would
 * just have been a fourth dialect.
 *
 * MEASURED BEFORE IT SHIPPED, against the real production corpus (161 briefs,
 * 159 post target keywords, 39 rejection entries): **zero** candidates change
 * verdict under any consumer, and **zero** become un-rejected. Unification is
 * behaviourally inert, which is the only honest form for a consolidation to take.
 *
 * That was only true after ONE data fix, and the divergence had been hiding it:
 * `"sodium lauryl sulfate"` was rejected as a standalone ingredient-explainer
 * topic ("no product mapping") with no `matchType`, so it matched as a SUBSTRING
 * and blocked `"toothpaste without sodium lauryl sulfate"` — a phrase this site
 * has a LOCKED WINNER ranking for (`toothpaste-without-sls`, 102,816
 * impressions/90d, the single biggest page on the blog). `content-strategist`
 * had been refusing to propose it all along; `brief-triage` had not, and only
 * because it ignored `matchType`. Narrowing that entry to `exact` is what makes
 * the two agree, and it is in the same change.
 *
 * @param {string} keyword
 * @param {Array<object|string>} rejections
 * @returns {boolean}
 */
export function isRejected(keyword, rejections = []) {
  const kw = String(keyword ?? '').toLowerCase().trim();
  if (!kw) return false;
  const kwSlug = rejSlug(keyword);
  return (rejections || []).some((raw) => {
    const entry = typeof raw === 'string' ? { keyword: raw } : (raw || {});
    const term = String(entry.keyword ?? '').toLowerCase().trim();
    if (!term) return false;
    if (entry.matchType === 'exact') return kw === term || kwSlug === rejSlug(entry.keyword);

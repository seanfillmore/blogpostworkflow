...
/**
 * Visible text only: no style/script, no markup, entities decoded, whitespace collapsed.
 *
 * Entity decoding is load-bearing, not tidiness. Klaviyo re-encodes on save — a rebuild
 * containing `Browse sets & bundles` comes back as `Browse sets &amp; bundles` — so
 * without this the text comparison reports a difference on a byte Klaviyo itself
 * introduced, and the in-sync check re-pushes an email that is already correct.
...
/**
 * Is the flow already serving this rebuild?
 *
 * `--all` re-offers every file on every run, and a push is never idempotent: it mints
 * a library template and a fresh snapshot even when the content is identical, so an
 * unguarded re-run churns live flows and strands a template per email for nothing.
 *
 * Compared on tags, links and visible TEXT rather than bytes, because Klaviyo rewrites
 * markup on save (pretty-printed CSS, stripped CSS comments, an inserted `<head>`), so
 * `live === intended` is false even one second after a verified push.
 */
export function alreadyInSync(intendedHtml, liveHtml) {
  if (!liveHtml) return false;
  const same = (a, b) => a.length === b.length && a.every((x, i) => x === b[i]);
  return (
    same(tagsIn(intendedHtml), tagsIn(liveHtml)) &&
    same(linksIn(intendedHtml), linksIn(liveHtml)) &&
    visibleText(intendedHtml) === visibleText(liveHtml)
  );
}

 ...
 * `formatsForAngle()` (agents/ad-brief/index.js) found for this angle's awareness level;
 ...
 * `gates` are carried over untouched (this is a parameter change, not a decision), and
 * if the brief happens to already be `approved`, writeBrief's own gates check runs
 * again against the unchanged `gates` block and passes, exactly as it did before.
 */
export function chooseFormat(root, product, briefId, formatKey) {

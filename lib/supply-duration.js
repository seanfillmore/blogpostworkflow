...
export function bindingDuration(components) {
  // Group by PRODUCT before taking the minimum.
  //
  // The first version took the minimum over component ROWS, which is right for a
  // mixed box and silently wrong for a multipack. A 4-pack of bar soap in four
  // scents is four rows of qty 1, so row-wise it reported 25 days — one bar —
  // when the box plainly holds four bars and ~100 days. Any cadence set from that
  // would ship four times too often.
  //
  // Grouping first is the general rule and needs no special case: Head-to-Toe's
  // seven single units group to seven products and behave exactly as before,
  // while the 4-pack groups to one product at qty 4.
  const byProduct = new Map();
  for (const c of components) {
    byProduct.set(c.product, (byProduct.get(c.product) ?? 0) + Number(c.qty ?? 1));
  }

  const detail = [], unknown = [];
  for (const [product, qty] of byProduct) {
    const r = rates[product];
    if (!r) { unknown.push(product); continue; }
    detail.push({ product, qty, daysPerUnit: r.daysPerUnit, days: r.daysPerUnit * qty, confidence: r.confidence });
  }
...
export function assertDurationClaim(claimedDays, components, label = 'this bundle') {
  ...
  if (claimedDays > days) {
    ...
    throw new Error(
      ...
      // Only gap-derived rates are upper bounds. A merchant estimate is a direct
      // figure, and telling someone "the true figure is lower still" about their
      // own product knowledge is both wrong and irritating.
      + (worst.confidence === 'merchant estimate'
        ? 'That figure comes from the merchant directly.'

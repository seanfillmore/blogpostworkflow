...
// ---------- Flow message content (revision-sensitive) ----------

/**
 * `PATCH /api/flow-actions/{id}` went GA in revision 2025-10-15. On anything older it
 * is not "forbidden" — it does not exist, and Klaviyo answers
 * `404 "No valid revisions found for method"`. That 404 is what fooled an earlier
 * audit into recording flow emails as permanently read-only: it tested 2024-10-15,
 * 2025-01-15 and 2025-07-15, all of which predate the endpoint, and read three
 * identical 404s as proof the limit was not a versioning artifact.
 *
 * Pinned separately from REVISION rather than bumping the module default, because
 * Klaviyo revisions are breaking: moving every call in the fleet to 2026-07-15 to
 * reach one endpoint would change response shapes for campaigns and profiles too.
 */
export const FLOW_ACTION_REVISION = '2026-07-15';

const atFlowRev = { revision: FLOW_ACTION_REVISION };

/** A flow action including its full `definition` — the sparse fieldset on listFlowActions omits it. */
export async function listFlowActionsWithDefinition(flowId) {
  const d = await klaviyoRequest('GET', `/flows/${flowId}/flow-actions/?page%5Bsize%5D=50`, null, atFlowRev);
  return d.data.map((a) => ({ id: a.id, ...a.attributes }));
}

export async function getFlowAction(id) {
  const d = await klaviyoRequest('GET', `/flow-actions/${id}/`, null, atFlowRev);
  return { id: d.data.id, ...d.data.attributes };
}

/**
 * Patch fields on a flow action's email message, preserving everything else.
 *
 * The API replaces the whole `definition`, so the current one must be read and
 * merged — sending only the changed key silently drops from_email, subject line and
 * tracking settings.
 *
 * Raw HTML is NOT accepted here: `body` returns
 * `400 'body' is not a valid field for the resource 'FlowEmail'`. To change content,
 * pass `template_id` pointing at a library template; see pushTemplateToFlowAction.
 */
export async function updateFlowActionMessage(actionId, patch) {
  const action = await getFlowAction(actionId);
  const definition = action.definition;
  if (definition?.type !== 'send-email') {
    throw new Error(`flow action ${actionId} is '${definition?.type}', not send-email`);
  }
  const next = {
    ...definition,
    data: { ...definition.data, message: { ...definition.data.message, ...patch } },
  };
  const d = await klaviyoRequest(
    'PATCH',
    `/flow-actions/${actionId}/`,
    { data: { type: 'flow-action', id: String(actionId), attributes: { definition: next } } },
    atFlowRev,
  );
  return { id: d.data.id, ...d.data.attributes };
}
...
/**
 * The template a flow message actually renders.
 *
 * This is the ONLY honest way to verify a content push. The flow holds its own
 * snapshot, so the library template you wrote and the template that sends are
 * different objects with different ids — updating the library one changes nothing
 * that ships, and reports success while doing it.
 */
export async function getFlowMessageTemplate(messageId) {
  const d = await klaviyoRequest('GET', `/flow-messages/${messageId}/template/`, null, atFlowRev);
  return { id: d.data.id, ...d.data.attributes };
}

/**
 * Flow-owned templates are not writable (PATCH is 404) but they ARE deletable.
 * That asymmetry is what makes the orphan sweep possible.
 */
export async function deleteTemplate(id) {
  return klaviyoRequest('DELETE', `/templates/${id}/`, null, atFlowRev);
}

export default {
  klaviyoRequest, REVISION, FLOW_ACTION_REVISION,
  ...
  listFlowActionsWithDefinition, getFlowAction, updateFlowActionMessage,
  getFlowMessageTemplate, deleteTemplate,
};

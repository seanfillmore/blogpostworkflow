...
function isCommercial(page, productHandles) {
  const set = productHandles instanceof Set ? productHandles : new Set(productHandles || []);
  if (/\/(products|collections)\//.test(page)) {
    // any handle fragment match, or it's a product/collection URL at all
    for (const h of set) if (h && page.includes(h)) return true;
    return /\/products\//.test(page); // product URLs are inherently commercial
  }
  for (const h of set) if (h && page.includes(h)) return true;
  return false;
}
...
export function analyzeOpportunities(rows, { productHandles = [] } = {}) {
  ...
  const opps = clusters.map((c) => {
    ...
    const commercial = isCommercial(c.page, productHandles);
    const upside = monthlyClickUpside(c, action);
    const score = Math.round(upside * (commercial ? PRODUCT_BOOST : 1));
    return { ...c, action, commercial, est_monthly_clicks: Math.round(upside), score };
  });

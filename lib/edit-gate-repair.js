...
import { getEditorReportPath, getContentPath, ROOT } from './posts.js';
...
export function runEditGateWithRepair(slug, { maxAttempts = 3, run, log = console.log } = {}) {
  ...
  // Every repair agent, and the editor re-check at the end of attemptRepair,
  // reads data/posts/<slug>/content.html. With no such file not one of them can
  // succeed, so looping here spends maxAttempts × (repairs + 1) subprocesses to
  // reach the verdict it started with. queue-autoapply did exactly that against
  // an orphaned post directory on 2026-08-26..28 and put six identical ENOENT
  // rows into the digest three days running.
  //
  // Only guard an ALREADY-FAILING gate. A post with no report and no body is a
  // page- or product-meta queue item, which has no article body by design and
  // must keep passing.
  if (!gate.pass && !existsSync(getContentPath(slug))) {
    log(`  ⊘ no local content.html for ${slug} — nothing to repair, skipping the loop`);
    return {
      gate: { pass: false, reason: `${gate.reason} — and there is no local content.html to repair` },
      attempts: 0,
    };
  }

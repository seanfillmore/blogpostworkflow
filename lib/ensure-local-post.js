...
import { resolvePostSlug, handleFromUrl, getContentPath, getMetaPath, ensurePostDir, replacePostMeta } from './posts.js';
...
export async function ensureLocalPostForUrl(urlOrHandle) {
  ...
  for (const blog of await getBlogs()) {
    ...
    replacePostMeta(handle, buildBootstrapMeta(article, blog));
    return handle;
  }
  return null;
}

 ...
 *
 * WHY: the dashboard runs under PM2, which does NOT source .env — verified
 * 2026-08-17 by reading /proc/<pid>/environ on the production box, where
 * FACEBOOK_ACCESS_TOKEN was absent. The entry route passed
 * `process.env.FACEBOOK_ACCESS_TOKEN` straight through, so every live entry
 * called this module with `accessToken: undefined` and returned a silent false.
 * The Lead events that DO exist in the dataset came from hand-run scripts, which
 * load .env — which is exactly why the endpoint looked verified and was not.
 *
 * The trap is worse than a plain missing variable: routes/meta-ads.js assigns
 * `process.env.FACEBOOK_ACCESS_TOKEN` when someone connects Meta through the
 * dashboard UI, so Leads fire for the life of that process and stop at the next
 * restart, with nothing to connect the two events.
 *
 * Same precedence CLAUDE.md documents for CREATIVES_BUDGET_BYTES, for the same
 * reason: the unattended path and the hand-run path must both find it.
 */

...
import { resolveMetaAccessToken } from './meta-token.js';
 ...
 */
export function resolveLeadAccessToken(env = process.env, opts) {
  return resolveMetaAccessToken(env, opts);
}
...
export async function sendLeadEvent({
  ...
  // NO `custom_data`, SO NO `value` AND NO `currency` — DELIBERATE, NOT AN OVERSIGHT.
  //
  // Events Manager raises three Diagnostics warnings about this, all on `Lead`,
  // all at 100% affected (seen 2026-09-21):
  //   "Send valid currency codes for more accurate ROAS calculation"  — currency missing
  //   "Send valid price and currency data for more accurate ROAS calculation" — value + currency missing
  //   ...and the same issue again under a generic "Details" panel.
  //
  // THEY CANNOT BE CLEARED HONESTLY. Meta's own requirement is that `value`
  // "must be a numeric value that is greater than 0", and the measured worth of a
  // giveaway entry is $0.00 — 7,128 entrants produced ZERO purchasers, verified
  // twice (Klaviyo's Placed Order stream and, independently, Shopify's orders,
  // which agreed on the same single address, and that order was itself $0.00).
  // Clearing the warning therefore means inventing a positive number for something
  // measured at nothing, and then letting Meta optimise delivery against it. A
  // fabricated conversion value is worse than an absent one: it is the same class
  // of error as the entry-page-vs-product-revenue mix-up that nearly had a $324.85
  // category condemned as a dud.
  //
  // Sending `currency` alone was considered and rejected — it clears one of the
  // three warnings, leaves the other two, and buys nothing.
  //
  // WHEN THIS SHOULD CHANGE: only once a lead type with a MEASURED entry→purchase
  // rate exists, at which point `value` is that rate × contribution per order.
  // A discount-code signup is a plausible candidate; a sweepstakes entry is not.
  //
  // Note the blast radius is currently zero either way: `sendLeadEvent` has exactly
  // one caller (agents/dashboard/routes/giveaway.js), the giveaway closed
  // 2026-09-14, and the Meta launch optimises for AddToCart rather than Lead —
  // so no Lead has fired since, and none is expected to.
  const event = {
    event_name: 'Lead',

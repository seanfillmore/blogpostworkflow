 ...
 * 3. `asynchronous_ai_overview: true` means the overview's content was NOT
 *    delivered in this response. That is NOT "an overview citing nobody" — it is
 *    no measurement at all, and folding it into the denominator manufactures a
 *    false zero.
 *
...
export function extractAiOverview(items) {
  ...
  if (!aio) return { present: false, asynchronous: false, text: '', references: [], domains: [] };
  ...
  return { present: true, asynchronous, text, references, domains };
}
...
export function summarizeQueryRuns(runs, targetDomain) {
  ...
  const list = runs ?? [];
  const resolved = list.filter((r) => r?.present && !r?.asynchronous);
  const unresolved = list.filter((r) => r?.present && r?.asynchronous);

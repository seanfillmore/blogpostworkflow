 ...
 */
// Every Schwartz level now maps to a format awareness level. `unaware` and `most-aware`
// were null until 2026-08-18 — no format covered them, which left 4 of 15 angles briefable
// but unrenderable, including the highest-scoring angle on file. `fact-hook` and
// `spec-panel` (agents/ad-studio/formats.js) close that.
//
// They map to their OWN values rather than onto the nearest of the original three. An
// `unaware` angle routed to a `problem` format would be an ad premised on the reader
// already knowing they have a problem, shown to a reader who by definition does not — a
// silent substitution, and the ad-brief README's whole argument for leaving the gap visible
// was that a wrong format is worse than a recorded absence.

...
/**
 * Strip the AI-assistant scaffolding GSC now returns wrapped around a real
 * question, and return the question alone.
 *
 * THIS IS NOT COSMETIC — it shipped into a feed. The 2026-09-07 `--all` run put
 * FOUR products' Q&A pairs into the TSV whose "question" began
 * `context: location: united states (not for language). do not include location
 * references in your response. question: …`. The ANSWERS were fine; the question
 * text was machine plumbing, and it would have gone to Google verbatim as
 * customer-facing copy.
 *
 * Measured over the same 28 GSC snapshots the feed reads: **20 queries carry it,
 * every one in the identical shape**, differing only in the country name. They
 * are individually tiny (1-5 impressions) and reach the top 30 only because
 * paraphrase collapsing sums them onto a survivor.
 *
 * STRIP, DO NOT WITHHOLD. What follows `question:` is a real buyer question with
 * real impressions — "i'm sensitive to strong fragrance—what are good options
 * for body lotion" — and dropping it would discard measured demand to avoid a
 * prefix. Cleaning also lets it dedupe against its own naturally-typed twins,
 * which is what should have happened to it in the first place.
 *
 * It matches the known shape ONLY: anything else passes through byte-identical,
 * the same whitelist doctrine as `PRODUCT_NOUNS` in lib/product-category-terms.js.
 * An unrecognised wrapper can therefore only ever produce a MISS — one ugly
 * question a human sees in the review file — never a mangled good one.
 */
const ASSISTANT_SCAFFOLD_RE = /^\s*context:\s*.*?\bquestion:\s*/is;

export function stripAssistantScaffolding(query) {
  const q = String(query ?? '');
  const m = ASSISTANT_SCAFFOLD_RE.exec(q);
  if (!m) return q;
  const rest = q.slice(m[0].length).trim();
  // A wrapper with nothing after it is not a question — keep the original so the
  // caller's own question test rejects it, rather than inventing an empty one.
  return rest || q;
}
...
export function extractQuestions(rows, { limit = Infinity } = {}) {
  ...
  for (const row of rows ?? []) {
    // Cleaned BEFORE the question test and before either dedupe: the scaffolding
    // opens with "context:", so the wrapped form fails `isQuestionQuery`'s
    // interrogative test and survives only on the trailing question mark.
    const query = stripAssistantScaffolding(row.query ?? row.keys?.[0]);

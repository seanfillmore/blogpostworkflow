...
export async function fetchClarityInsights({ url: pageUrl = null, numOfDays = 1 } = {}) {
  ...
  // Clarity's bot count is not reliably a subset of totalSessionCount — on
  // 2026-07-19 it reported 1,037 bots against 66 total sessions, which made
  // `real` -971 and silently corrupted every trend built on it. We do not know
  // whether totalSessionCount excludes bots on those days or the two counts
  // cover different windows, so rather than guess a formula we clamp at zero and
  // flag the day as unreliable, keeping both raw counts for later re-derivation.
  const botsExceedTotal = bots > totalSessions;

  return {
    sessions: {
      total: totalSessions,
      bots,
      real: Math.max(0, totalSessions - bots),
      realUnreliable: botsExceedTotal,

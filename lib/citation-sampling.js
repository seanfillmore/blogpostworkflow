...
export function aggregateRuns(runs) {
  ...
  return {
    ...
    // ADDITIVE (2026-09-21). Gemini's grounding redirector resolved to the real
    // publisher; empty for every engine that cites directly, and empty on a
    // `--no-resolve` run. Stored because the tokens EXPIRE after ~30 days, so
    // this is the only thing that keeps a snapshot's Gemini citations legible
    // once they do. Note this is a UNION across runs, so it can say WHETHER a
    // cell cited us but never how many runs did — that is `cited_runs`, which
    // the agent computes per run before aggregating.
    citation_urls_resolved: union(ok.map((r) => r.citation_urls_resolved)),

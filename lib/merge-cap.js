...
export function capMergesPerWinner(decisions, { maxPerWinner = MAX_MERGES_PER_WINNER } = {}) {
  ...
  for (const decision of decisions || []) {
    const merges = mergeCount(decision);
    ...
    const key = winnerKey(decision);
    const used = perWinner.get(key) || 0;

    if (used + merges > maxPerWinner) {
      deferred.push({ decision, winner: decision.winner, merges });
      continue;
    }
  ...
  return { apply, deferred, perWinner };
}

/** Console/digest lines describing what the cap held back. */
export function mergeCapLines({ deferred = [], perWinner = new Map(), maxPerWinner = MAX_MERGES_PER_WINNER } = {}) {
  if (!deferred.length) {
    return [`· Merge cap: no winner reached ${maxPerWinner} body rewrites this run; nothing deferred.`];
  }

...
## Decide the stage on EVERY tactic you adopt

For each tactic you adopt, ask one more question before you finish it: **could a solo
operator execute this today, with no ad budget, no media buyer, and no team?**

- **Yes** → omit \`stage\`. This is the common case.
- **No** → keep the adopt verdict and set \`stage\` to the gate that unblocks it.

This is a required step on the adopt path, not an alternative to rejecting. If your
own \`rscFit.reasoning\` says a tactic needs budget, staff, traffic volume, or working
attribution — or uses words like "park", "later", "once we reach", "when the business
has" — then \`stage\` MUST be set. Reasoning that describes a gate while \`stage\` is
null is a contradiction, and it ships a tactic this business cannot run into the live
menu the ad-copy agent draws from.

A tactic blocked ONLY by timing is adopted with a \`stage\` rather than rejected. Do
not reject for stage reasons.

Set \`"stage"\` on the adopted tactic to the gate that unblocks it:

| stage | Set it when the tactic needs |
|---|---|
| \`"tracking"\` | working attribution before it can be judged |
| \`"cro"\` | conversion-rate work on pages that already get traffic |
| \`"offer-aov"\` | a bundle, offer, or price structure that does not exist yet |
| \`"traffic"\` | ad budget, a media buyer, a large list, or traffic volume this business lacks |

Omit \`stage\` entirely for anything executable today — that is the common case, and a
tactic wrongly parked is invisible to the fleet until someone goes looking.

Parking is not a softer reject. The tactic still has to clear the bar for everything
else: a real mechanism, no duplication, and an honest fit here. Score it on its merit
when its gate opens, not on today's inability to run it — a 7/10 tactic blocked only by
budget is a parked 7, not a 3.
...
export const CURRENT_STAGE = 'cro';

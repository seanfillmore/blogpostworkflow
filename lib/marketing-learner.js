...
- **Meta budget: $30/day (~$900/mo)**, hand-run by one person. That is real money against
  ~$2,700/mo combined revenue, and it is also the number to judge against: a tactic
  assuming a $100/day floor, a 20-creative test cell, an agency, or a media buyer fails on
  THAT constraint — which is real and durable — never on a gate, which is not.
...
- Has no honest translation to a solo-operator ecommerce catalog (B2B service pricing,
  physical-location expansion, agency retainers, "move to a bigger market").
- Targets a platform Real Skin Care is not on and has no plan to be on.

## Do not set a stage — every gate is now open

\`stage\` parks a tactic that is right for this business but blocked by the operating
sequence. **All four gates (tracking, cro, offer-aov, traffic) are open as of 2026-08-17**,
so there is no longer a gate to park behind. **Omit \`stage\` on every tactic.** Setting it
is now a no-op that only makes the report harder to read.

The judgement it used to carry still matters, but it belongs in the score and the verdict:

- **Executable now, with a real mechanism, not a duplicate** → adopt, and score on merit.
- **Assumes resources this business does not have** — an agency, a media buyer, a designer,
  a paid analytics subscription, a large daily spend, a 20-creative test cell — → that is a
  standing constraint, not a phase that will pass. Judge it on that. A tactic with an honest
  scaled-down version is an adopt at a lower score with the scale-down stated; one with no
  honest scaled-down version is a reject.
- **Needs volume to read** (statistical significance at ~54 orders/month, a sub-1,000-person
  list) → adopt if the mechanism is sound, and say plainly in the fit that conclusions will
  be directional rather than significant.
 ...
 */
export const STAGES = Object.freeze(['tracking', 'cro', 'offer-aov', 'traffic']);
 ...
 * entries to the soap giveaway, so the last gate is open and every parked tactic goes
 * live. Note the consequence: 'traffic' is the final stage, so `isStageActive` now returns
 * true for everything and parking is a no-op until a later gate is appended to STAGES.
 * `buildConstraintBlock` tells the extractor to stop setting `stage` for that reason.
 */
...
export async function extractTactics({ video, inventory = [], client, chunk = null, maxTokens = 32000, retryOptions = {} }) {
  ...
  // it; three consecutive runs of $100M Money Models died that way. Truncation and
  // schema failures are deterministic, though — retrying those would pay for four
  // calls and three minutes of backoff to reach the identical error.
  ...
  try {
    ...
    // Same contract as the schema failure below, plus stop_reason: when the text is
    // empty, why the model stopped IS the diagnosis.
 ...
 *
 * Budgeted at 32k, and streaming, for the same reason extraction is — but the
 ...
 * sized when these skills were young; marketing-paid-acquisition-scaling reached
 * 4,339 words and overflowed it, killing a run after the transcript and extraction
 ...
 * would have failed on whichever skill grew fastest. max_tokens is a ceiling rather
 ...
 */
export async function mergeSkillContent({ existingContent, tactics, client, maxTokens = 32000 }) {
  ...
  const stageRule =
    `\n- A "**Stage:** <gate>" line directly under a heading means that tactic is parked until that\n` +
    `  phase of the business opens. Keep every such line EXACTLY where it is and worded as it is.\n` +
    `  Removing one silently promotes a tactic this business cannot execute yet.` +
    (staged.length
      ? ` Currently parked here: ${staged.map((t) => `"${t.claim}" (${t.stage})`).join(', ')}.\n`
      : '\n') +
    ...
    `- A parked section is REMOVED from the projection the agent fleet reads. So: do not mention a\n` +

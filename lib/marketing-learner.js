 ...
 * every chunk: a 3-chunk transcript produced 37 candidates, whose consolidated prose
 * did not fit in 16k and tripped the truncation guard. max_tokens is a ceiling, not a
 * charge — a short run bills the same. The larger budget forces streaming (see below);
 * lib/anthropic.js meters stream() as well as create() so this stays in the cost report.
 */
export async function consolidateTactics({ candidates, source, client, maxTokens = 32000 }) {

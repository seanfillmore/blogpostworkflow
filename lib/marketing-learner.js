...
export function buildRegatePrompt({ items = [] } = {}) {
  ...
  return `Every tactic below is ALREADY ADOPTED and live in a skill. Decide, for each, whether it
should instead be parked behind a stage gate.

${buildConstraintBlock({ sourceType: 'file', sourceKind: 'book' })}

## What you are deciding

Only one thing: does this tactic need a gate, and which one. Do NOT re-score, do not rewrite
the claim, do not propose moving it to another skill, do not reject it. It is adopted; the
only question is whether it is runnable today.
...
- \`"team"\` — needs a person who is not the solo operator: an agency, a media buyer, a
  designer, an editor, a videographer, a creator roster, sales or support staff, or anyone
  to delegate to or train.
...
export async function regateAdopted({ items, client, maxTokens = 32000, retryOptions = {}, deadlineMs = STREAM_DEADLINES_MS.regate }) {
  ...
  return decisions
    ...
    .map((d) => ({ ...items[d.index], stage: d.stage, reasoning: d.reasoning }));
}
...
/**
 * A wall-clock deadline for one streaming request.
 *
 * On 2026-08-23 a consolidation call sat for 71 minutes having burned 3.84 seconds of
 * CPU with its socket still open — a stream that was not arriving. Nothing caught it:
 * an HTTP client's timeout is a per-chunk read timeout, not a wall clock, and withRetry
 * cannot fire on a promise that never settles. Unattended on the 1st of the month, that
 * is a cron job that stops without failing.
 *
 * AbortController + setTimeout, NOT AbortSignal.timeout(). The latter's timer is unref'd,
 * so it does not hold the event loop open — on Node 22 that is exactly what let a
 * stubbed-fetch test exit before its timeout fired and report `cancelled` alongside
 * `# fail 0`, a dead test that reads like a pass. Same shape as lib/google-ads.js.
 *
 * The timer MUST be cleared on the happy path too, or a finished run holds the event
 * loop open until the deadline elapses.
 */
export function streamDeadline(ms, label = 'stream') {
  ...
  const timer = setTimeout(() => {
    ...
    // NON-RETRYABLE, deliberately. withRetry treats 4xx-except-429 as terminal, and this
    // opts into that: a stalled stream must fail the run, not grind on it.
    //
    // Retrying is what makes a deadline actively harmful here. withRetry does 3 attempts,
    // then waits 30 MINUTES and restarts the whole cycle, up to 10 times — so feeding a
    // 25-minute consolidation deadline into it turns one 71-minute hang into a multi-hour
    // retry storm occupying the cron slot. Failing fast costs almost nothing by comparison,
    // because chunk extraction is cached: the 2026-08-23 re-run after a kill served all 16
    // chunks from cache and only re-paid the one call that stalled.
    err.status = 408;
...
async function streamWithDeadline(client, params, { ms, label }) {

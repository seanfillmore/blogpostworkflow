...
/**
 * The business reality every tactic is scored against. The measured figures are
 * INTERPOLATED from `lib/business-baseline.js`, never spelled here — this block and
 * `lib/bing-keyword-gap.js` each carried their own hardcoded $50.46 and both drifted
 * 8.5% before anyone noticed. That module also carries the expiry; note the AOV is the
 * trailing-90d figure, NOT the all-time $19, which is dragged down by a 2024 order
 * spike and must never be used for decisions.
 */
/**
 * When the paid-spend status in the constraint block was last CHECKED AGAINST REALITY.
 *
 * This block scores every tactic the fleet ever ingests, and it has gone stale silently
 * before: through 2026-09-05 it still read "the traffic gate is OPEN as of 2026-08-17" and
 * "Meta budget: $30/day" when spend had been paused on 2026-08-27. Nothing said so, because
 * prose in a template cannot expire.
 *
 * `assertPaidStatusFresh` is the fix — a test calls it, so an ageing figure fails CI instead
 * of quietly mis-scoring runs. It does NOT check whether the status is CORRECT (nothing here
 * can reach the Graph API); it checks that a human has re-asserted it recently. Verify at all
 * three levels — campaign, ad set, ad — before updating the date; an Ads Manager campaign row
 * reads convincingly active while the rows beneath it are off, which has already produced one
 * false "campaign is live" report here.
 */
export const PAID_STATUS_AS_OF = '2026-08-27';
export const PAID_STATUS_MAX_AGE_DAYS = 45;

export function assertPaidStatusFresh(now = new Date()) {
  const age = Math.floor((now - new Date(`${PAID_STATUS_AS_OF}T00:00:00Z`)) / 86_400_000);
  if (age > PAID_STATUS_MAX_AGE_DAYS) {
    throw new Error(
      `The paid-spend status in buildConstraintBlock was last verified ${age} days ago `
      + `(${PAID_STATUS_AS_OF}), past the ${PAID_STATUS_MAX_AGE_DAYS}-day limit. Every tactic `
      + `is scored against it. Re-check the Meta campaign at CAMPAIGN, AD SET and AD level, `
      + `update the block to match, and move PAID_STATUS_AS_OF.`);
  }
  return { age, stale: false };
}

/**
 * WHO DOES THE WORK — the operating reality every stage decision rests on.
 *
 * Written once and interpolated into the extraction block, the re-gate prompt and the
 * re-adjudication prompt, because three hand-copied definitions of `team` is three
 * definitions that drift. Same rule as AWARENESS_LEVELS being imported, never redeclared.
 *
 * THE DISTINCTION THIS EXISTS TO DRAW: `team` is NOT a list of skills the operator lacks.
 * He performs every craft role himself — media buying, editing, design, videography,
 * scripting, shooting, copywriting. What he cannot be is a SECOND PERSON. Before 2026-09-06
 * the gate listed "video or UGC production, a designer, an editor, a media buyer" as things
 * to park, which parked a phone-shot vertical video and every media-buying tactic behind a
 * hire that is not coming. That is the failure mode this file already documents for the
 * traffic gate — sound work hidden from the fleet on a constraint that was not real.
 */
/**
 * WHERE THIS BUSINESS IS GOING — stated before the current figures, deliberately.
 *
 * Sean, 2026-08-25, verbatim: "We are not constrained to small budgets forever. If we have
 * a campaign that is making sales, we can scale up to get out of learning. You frame
 * everything as doom and gloom instead of setting goals from where we are to where we need
 * to be." Restated 2026-09-08: "Stop basing every decision on $30/day. Think big picture.
 * We want to scale as big as possible."
 *
 * MEASURED CONSEQUENCE OF GETTING THIS WRONG (2026-09-08): 64 Fit rationales across 19
 * skills cap a score citing volume or budget — "held at 6 … nothing here is measurable at
 * ~54 orders/month" and 63 like it. That is forbidden by the stage rules' own words: the
 * stage marker already records timing, so discounting the score double-counts it. And it is
 * not cosmetic — 489 scores reach the fleet projection creative-packager reads as its
 * copy-tactic menu, so a tactic capped at 5 instead of 8 is handed to the copy agent as
 * weaker guidance than it deserves.
 */
export const GROWTH_INTENT_RULE = `## THE TARGET IS $1M/YEAR. The figures below are a starting line

**Named goal (Sean, 2026-09-08): $1M/yr — about $83K/month.** Today the business runs
~$2,700/mo combined (Shopify ~$875 + Amazon ~$1,800), so the target is roughly a **30×
multiple**, not a 20% improvement. Score tactics against the business this is trying to
become, not the one in the current-state list.

**A 30× goal changes what "relevant" means.** A tactic that only pays at volume this business
does not have yet is ON the path, not off it — that is what the \`stage\` markers are for.
What is genuinely off-path is a tactic with no honest translation to an ecommerce catalogue
at ANY size. Judge against that, not against this month's numbers.

**The route, so a threshold reads as a milestone rather than a wall.** The engine is: acquire
the SKIN line with paid (lotion/cream/sets carry $15–25 contribution per order, so they can
fund acquisition) → lift AOV so a first order roughly pays for itself → harvest the
low-margin catalogue through retention. **Deodorant, soap, toothpaste and lip balm carry
$2–5 after shipping — they are retention and AOV plays, never a cold-paid front end.** The
sequencing spine is Tracking → CRO → Offer/AOV → Traffic, and **all four gates are open**.
The binding constraint is CONVERSION RATE, not capital and not capacity.

**Every number in this section is where it stands TODAY, not a ceiling, and never grounds
for discounting a tactic.** Budget, list size, order volume and headcount are all variables
the business is actively working to move.

**NEVER LOWER A SCORE BECAUSE OF CURRENT VOLUME, BUDGET OR MEASURABILITY.** Not "held at 6
because nothing here is measurable at this volume", not "capped at 5 because the list is
small", not "not higher because we cannot run this yet". Score every tactic on the merit it
has when it is working at the scale this business is driving toward. **Timing is recorded in
exactly one place — the \`stage\` marker — and nowhere else.** Recording it twice buries good
work: the tactic sorts lower in the fleet's own projection AND is hidden behind a gate.

When a threshold genuinely blocks something, the useful answer is the ROUTE, not the wall:
where we are now, the number that clears it, and the stages between. "Exiting the learning
phase needs ~50 conversions/week, which is $X/day at today's conversion rate — here are the
milestones" is useful. "We cannot exit learning at this volume" is not.`;

/**
 * The `team` gate, in ONE place. Interpolated wherever a stage decision is made.
 *
 * The test is ANOTHER PERSON'S TIME, never a skill or a job title. Someone else's hours,
 * someone else's face, or someone else's employment — those are the only things the operator
 * cannot substitute by doing the work himself.
 */
export const TEAM_GATE_RULE = `- \`team\` — **needs ANOTHER PERSON'S TIME, FACE OR EMPLOYMENT.** Not a skill, not a job title:
  the operator performs every craft role himself (see the capability rule above). Park here
  only what a second human being has to be present for — **hiring, recruiting, interviewing,
  onboarding, training or delegating; paying an agency, freelancer or contractor a retainer;
  a roster of creators, influencers or hired actors appearing as themselves; a review or
  approval chain; sales or support staff; managing or coaching anyone.**
  The test in one line: *"one person films, cuts and posts it"* is runnable TODAY;
  *"brief three creators and route the winners to your editor"* is \`team\`.
  **Judge the tactic by what CARRYING IT OUT requires, not by whether its first step is solo
  research.** Screening agencies, shortlisting creators, classifying influencer tiers and
  browsing for talent are all things one person can do at a laptop — and every one of them
  exists to ENGAGE somebody. The outcome is the retainer or the roster, so the tactic is
  \`team\`. A tactic is un-parked only when the operator can run it *to its stated outcome*
  alone; being able to do the reconnaissance alone is not enough.
  **Naming a role is NOT naming a person.** A tactic addressed to "your media buyer", "your
  editor", "your designer" or "your videographer" is addressed to HIM — translate it and adopt
  it unparked. Park it only if it needs that person to be someone else.`;

export const SOLO_CAPABILITY_RULE = `- **A SOLO OPERATOR RUNS THIS BUSINESS — no team, no agency, no staff — AND HE DOES EVERY
  ROLE HIMSELF. That is the capability, not the limit.** The solo operator is the media
  buyer, the editor, the designer, the videographer, the scriptwriter, the copywriter, the
  analyst and the person who hits publish. He runs the ad account, cuts the video, builds the
  creative, films himself and writes the copy. **Never park a tactic behind
  \`team\` because it names one of those ROLES** — naming a role is not naming a second person.
  "Your media buyer should…", "have your editor…", "the designer will…" all translate to *he
  does it*, and the tactic is runnable today.
- **ORGANIC SHORT-FORM VIDEO IS A LIVE SURFACE he runs himself** — RSC posts its own social
  video; he films, edits, writes and publishes it. "It is a video" is never by itself a reason
  to park. A phone, a script and one person is today's production capability. **A tactic from a
  video source must be translated to RSC's own social channels FIRST** — the spoken and
  on-screen hook it was actually written about — and only then to Meta primary text, email
  subject lines and Amazon bullets. A video-craft tactic silently reworded into a
  written-copy-only tactic has lost the surface it came from.
- **PAID MEDIA IS HIS JOB TOO.** Campaign structure, budget decisions, bid strategy, audience
  setup, reading the reporting, killing an ad, scaling one — all runnable today. Park a paid
  tactic behind \`scale\` only for genuine VOLUME (a $100/day floor, a multi-cell test, a
  readable statistical signal, a paid analytics subscription), never behind \`team\`.`;
...
- **Measured ${BASELINE_AS_OF}** (trailing 90 days, live paid orders). Shopify revenue
  ~$${SHOPIFY_MONTHLY_REVENUE}/mo. Amazon ~$1,800/mo. Combined ~$2,700/mo.
- **Order volume: Shopify ~${SHOPIFY_MONTHLY_ORDERS} orders/month. ~54/month is Shopify PLUS Amazon.** Use ~${SHOPIFY_MONTHLY_ORDERS} for
  anything about the Shopify funnel — learning-phase maths, conversion arithmetic, whether
  a test can read. Quoting 54 there overstates Shopify volume by roughly 3×.
- AOV $${AOV_TRAILING_90D.toFixed(2)} (trailing 90 days, incl. shipping and tax). At this
  volume the MEDIAN order is far below the mean — a handful of large orders carry a whole
  window — so this is the right multiplier for a forecast and the wrong number to call
  typical.
...
${SOLO_CAPABILITY_RULE}
- **The traffic gate is OPEN** — and it stays open whether or not money is moving. The ad
  account, pixel, tracking, audiences, ad set and creative all EXIST and have delivered.
  ...
  false. It is exactly how six directly-relevant Meta reporting tactics were scored
  4/10 and hidden from the fleet on 2026-08-16, the same week the account was being built.
- **PAID SPEND IS CURRENTLY PAUSED (since ${PAID_STATUS_AS_OF}), and that is a BUDGET
  decision, not a closed gate.** The giveaway campaign ran $259.20 over 9 days for 1,447
  leads at $0.179 each — 14× better than target — and was paused because none of those
  3,071 entrants had bought anything yet, not because it failed. It is paused at CAMPAIGN
  level only; restarting is a single API call, and the entry-to-purchase measurement that
  decides whether to restart lands mid-September. **So paid tactics are still judged as
  runnable**: the operator can plan, write, structure and stage paid work today, and the
  spend resumes on one decision. What a paused budget genuinely blocks is only work that
  needs live delivery to produce a reading — park THAT behind \`scale\`, never \`traffic\`.
- **Reference budget when it was last running: $30/day.** That is the number for ARITHMETIC
  — what a test costs, how long a reading takes — and it is **NOT a ceiling on ambition and
  never a reason to discount a tactic.** Budget is the most changeable variable here: a
  campaign that makes sales gets scaled up. A tactic assuming a $100/day floor, a
  20-creative test cell, an agency retainer or a hired crew is **early, not wrong** — park
  it behind \`scale\` or \`team\` per the stage rules below, at its FULL merit score. (A
  "media buyer" or "video crew" in a source's own wording is NOT a gate — he is both.)
...
- **Sound, but blocked only by volume, budget or people** → **adopt with the stage that
  ...
  **This is the rule most often broken, so it is worth restating as a ban on a SENTENCE.**
  Do not write "held at N because nothing here is measurable at this volume", "capped at N
  because the list is small", or "not higher because we cannot run it yet" — in any wording.
  A measured 64 rationales across 19 skills did exactly that, and those scores are read by
  the agent that writes ad copy. If the only thing holding a tactic back is scale, budget,
  volume or headcount, the score does not move at all — only the \`stage\` field does.
 ...
 */
/**
 * A staged tactic's claim with any leading ordinal stripped, for matching it across an
 * edit that renumbered it. Deliberately narrow — an ordinal prefix and nothing else, so
 * two genuinely different claims never collapse onto the same key.
 */
export function normalizeStagedClaim(claim) {
  return String(claim ?? '')
...
/**
 * Vocabulary that means "this needs a person who is not the solo operator, or a budget
 * line that does not exist". Matched against a NEW skill's name and description only —
 * deliberately not its tactic bodies, where "agency" shows up in passing prose.
 */
const TEAM_SHAPED = /\b(agenc(?:y|ies)|hir(?:e|ing)|staff(?:ing|ed)?|delegat(?:e|ing|ion)|employee|contractor|freelancer|media buyer|department head|payroll|salar(?:y|ies)|outsourc(?:e|ing))\b/i;

/**
 * Refuse to CREATE a skill whose whole subject is work this business cannot staff or fund
 * when not one of its tactics carries a stage.
 *
 * The stage field was absent from the extraction schema until 2026-08-23, so first-pass
 * ingestion could not park anything however emphatic the prose was. "$100M Leads" created
 * nine skills and 71 tactics, every one unparked — including seven on hiring agencies and
 * three on training a team, for a solo operator at ~54 orders/month on $30/day. Nothing
 * failed; it merged clean and was only caught by reading the result.
 *
 * The schema gap is fixed, but a schema the model CAN fill is not a schema it always fills
 * correctly, and this is the cheap structural check that would have caught that day on the
 * day. Deliberately narrow, to stay a signal rather than a nuisance:
 *   - New skills only. Existing ones are re-checked by nobody and would fire forever.
 *   - Name and description only, not tactic bodies.
 *   - ZERO staged tactics. One staged tactic proves the model weighed the question, and a
 *     skill can legitimately mix a solo scale-down ("pay a practitioner by the hour to
 *     teach you") with gated work ("hire two agencies in sequence").
 */
/**
 * Ask, for tactics ALREADY ADOPTED and live, which of them should have been parked.
 *
 * Needed because the extraction schema had no `stage` field until 2026-08-23 — the prose
 * argued for parking at length while the JSON the model filled in had nowhere to say it.
 * Every tactic ingested before that date was structurally ineligible for a gate, so the
 * live projection is under-parked by construction, not by judgement.
 *
 * STAGE-ONLY: scores are never revisited. That is not merely a smaller diff — the stage
 * rules say to score a parked tactic "on the merit it will have when that gate opens, not
 * on today's inability to run it", so a pass that moved scores would be double-counting
 * the timing the marker already records.
 *
 * Merged back positionally, by the `[n]` index the prompt assigns — never by echoing the
 * claim text, which is the failure the demand-miner classifier already paid for (a curly
 * apostrophe, a retained prefix and collapsed whitespace each broke the join).
 */
export function buildRegatePrompt({ items = [] } = {}) {
  const list = items.map((it, i) =>
    `[${i}] skill: ${it.skill}\n    claim: ${it.claim}\n`
    + `    currently: ${it.stage ? `PARKED behind \`${it.stage}\`` : 'runnable today (no gate)'}\n`
    + `    recorded fit: ${it.fit ?? '(none recorded)'}`
  ).join('\n\n');

  return `Every tactic below is ALREADY ADOPTED and live in a skill. Decide, for each, which gate
it belongs behind — **including none**.

Each tactic states whether it is currently parked. **You may move it in EITHER direction.**
Un-parking is a first-class outcome, not an exception: these gates have been mis-stated
before, and a tactic parked behind a capacity that actually exists is invisible to the whole
fleet for no reason. If a parked tactic is runnable today, return \`null\` and say so.

${buildConstraintBlock({ sourceType: 'file', sourceKind: 'book' })}

## What you are deciding

Only one thing: which gate this tactic needs, or none. Do NOT re-score, do not rewrite the
claim, do not propose moving it to another skill, do not reject it. It is adopted; the only
question is whether it is runnable today — asked fresh, ignoring what it is parked behind now.

- \`null\` — one person can run this today at ~54 orders/month, a sub-1,000 list and $30/day.
- \`"scale"\` — needs volume: a readable signal (significance, decile contrasts, frequency
  thresholds), a multi-cell test, a $100/day floor, or a paid analytics subscription.
${TEAM_GATE_RULE}

The four funnel gates (tracking, cro, offer-aov, traffic) are all OPEN — never park behind
those. Only "scale" and "team" are still ahead.

Be conservative in one specific direction: if a tactic has an honest scaled-down version a
solo operator can run today, it stays \`null\`. Paying a practitioner by the hour to teach
you is runnable; hiring two agencies in sequence is not. Parking something runnable buries
it, which is the opposite of what gates are for.
...
{
  "decisions": [
    { "index": 0, "stage": ${STAGES.map((s) => `"${s}"`).join(' | ')} | null, "reasoning": "one sentence — for a gate, name the specific capacity that is missing" }
  ]
}

Rules:
- One entry per tactic, every index present exactly once, matching the [n] above.
- reasoning is required when stage is non-null, and may be omitted when stage is null.`;
}

/**
 * Find live tactics whose Fit rationale CAPS the score on volume, budget or measurability.
 *
 * This is the defect PR #842 stopped creating and could not clean up: 64 rationales across
 * 19 skills say things like "held at 6 … nothing here is measurable at ~54 orders/month".
 * The stage rules forbid exactly that — the `stage` marker already records timing, so
 * discounting the score double-counts it — and the scores are not decorative: they reach the
 * fleet projection creative-packager reads, and adopted tactics sort by score. A tactic
 * capped at 5 instead of 8 is buried twice.
 *
 * PURELY A DETECTOR. It proposes nothing and rewrites nothing; `rescoreCapped` does the
 * judging. Regex-only so the expensive model call is spent on real candidates, and it
 * deliberately requires BOTH halves — a capping verb AND a scale reason — because "held at
 * 6 because it is a formatting convention" is a legitimate merit judgement that must not be
 * touched.
 */
const CAP_VERB = /\b(capped at|held at|not higher because|held back|no higher than|only a \d+ because)\b/i;
const SCALE_REASON = new RegExp([
  'orders?/month', 'orders per month', '\\$\\d+/day', 'sub-1,?000', 'small list',
  'never be measured', 'not be measured', 'unmeasurable', 'never measured',
  'too small', 'low volume', 'at this volume', 'at this scale', 'this budget',
  'cannot run', 'can not run', 'no budget', 'not enough traffic', 'not enough volume',
].join('|'), 'i');

export function findCappedScores(inventory) {
  const out = [];
  for (const s of inventory) {
    for (const section of s.content.split(/^(?=## )/m)) {
      const heading = section.split('\n')[0].replace(/^##\s*/, '').trim();
      if (!heading || /^Falsified/i.test(heading)) continue;
      const fit = section.match(/\*\*Fit here \(([^)]*)\):\*\*\s*([\s\S]*?)(?=\n\n\*|\n\*Source:|$)/);
      if (!fit) continue;
      const [, scoreRaw, reasoning] = fit;
      // Only the sentence doing the capping matters; a stray "$30/day" elsewhere is fine.
      const sentence = reasoning.split(/(?<=\.)\s+/).find((x) => CAP_VERB.test(x) && SCALE_REASON.test(x));
      if (!sentence) continue;
      const score = Number((scoreRaw.match(/(\d+)\s*\/\s*10/) ?? [])[1]);
      if (!Number.isInteger(score)) continue;
      out.push({ skill: s.name, claim: heading, score, sentence: sentence.trim(), fit: reasoning.trim().slice(0, 600) });
    }
  }
  return out;
}
...
/**
 * Re-score capped tactics. Returns ONLY those whose score should RISE.
 *
 * Deliberately one-directional, unlike `regateAdopted`. This pass exists to undo a specific
 * systematic error, and letting it also LOWER scores would turn a targeted repair into an
 * open re-judgement of the corpus on a model's whim — with no second source to check it
 * against. If a tactic's score is right where it is, the answer is "leave it".
 */
export function buildRescorePrompt({ items = [] } = {}) {
  const list = items.map((it, i) =>
    `[${i}] skill: ${it.skill}\n    claim: ${it.claim}\n    current score: ${it.score}/10\n`
    + `    the capping sentence: "${it.sentence}"\n    fit reasoning: ${it.fit}`
  ).join('\n\n');

  return `Each tactic below is ALREADY ADOPTED, and its recorded score was LOWERED for a reason
that is not allowed: current order volume, current budget, list size, or "we could not measure
it at this scale".
...
For each tactic: **what would this have scored if the person writing it had judged it on
MERIT alone** — how well it works, how well it translates to this catalogue, how durable it
is — with today's volume, budget and measurability taken completely off the table?

Timing belongs in the \`stage\` marker, which already exists. It must not be priced into the
score as well.

- Return a HIGHER score when the capping sentence was the only thing holding it down.
- Return the SAME score when the tactic is genuinely mediocre for reasons that have nothing
  to do with scale — it is a minor formatting convention, it overlaps something else, the
  evidence is thin. Those are real merit judgements and they stand.
- **Never return a LOWER score.** This pass only repairs; it does not re-open settled work.
- Do not rewrite the claim, do not change the skill, do not reject anything.
...
{
  "decisions": [
    { "index": 0, "score": 8, "reasoning": "one sentence — what the merit-only judgement is" }
  ]
}

Rules:
- One entry per tactic, every index present exactly once, matching the [n] above.
- \`score\` is an integer 1-10 and may never be below the current score.
- \`reasoning\` is required whenever the score changes.`;
}

export async function rescoreCapped({ items, client, maxTokens = 32000, retryOptions = {}, deadlineMs = STREAM_DEADLINES_MS.regate }) {
  ...
  const res = await withRetry(
    () => streamWithDeadline(
      client,
      {
        ...
        messages: [{ role: 'user', content: buildRescorePrompt({ items }) }],
      },
      { ms: deadlineMs, label: 'rescore' },
    ),
    { label: `rescore ${items.length}`, ...retryOptions },
  );
  if (res.stop_reason === 'max_tokens') {
    throw new Error(`Re-score of ${items.length} tactics hit max_tokens — output truncated. Refusing to apply.`);
  }
  const text = (res.content ?? []).filter((b) => b.type === 'text').map((b) => b.text).join('');
  const parsed = parseJsonBlock(text, `the re-score response for ${items.length} tactics`);
  const decisions = parsed?.decisions;
  if (!Array.isArray(decisions)) throw new Error('Re-score response has no decisions array.');

  const seen = new Set();
  for (const d of decisions) {
    if (!Number.isInteger(d.index) || d.index < 0 || d.index >= items.length) {
      throw new Error(`Re-score returned index ${JSON.stringify(d.index)}, outside 0..${items.length - 1}.`);
    }
    if (seen.has(d.index)) throw new Error(`Re-score returned index ${d.index} twice.`);
    seen.add(d.index);
    if (!Number.isInteger(d.score) || d.score < 1 || d.score > 10) {
      throw new Error(`Re-score returned score ${JSON.stringify(d.score)} for [${d.index}]; expected an integer 1-10.`);
    }
    // The one-directional guarantee is enforced in CODE, not left to the prompt.
    if (d.score < items[d.index].score) {
      throw new Error(
        `Re-score tried to LOWER [${d.index}] from ${items[d.index].score} to ${d.score}. `
        + `This pass repairs capped scores; it may never reduce one.`);
    }
    if (d.score !== items[d.index].score && !d.reasoning) {
      throw new Error(`Re-score changed [${d.index}] with no reasoning.`);
    }
  }
  if (seen.size !== items.length) {
    const missing = items.map((_, i) => i).filter((i) => !seen.has(i));
    throw new Error(`Re-score skipped ${missing.length} tactic(s): [${missing.slice(0, 10).join(', ')}].`);
  }

  return decisions
    .filter((d) => d.score > items[d.index].score)
    .map((d) => ({ ...items[d.index], newScore: d.score, reasoning: d.reasoning }));
}

/**
 * Raise one live tactic's `**Fit here (N/10):**` score by text surgery.
 *
 * Same discipline as insertStageMarker and falsifyTactic: one line changes, no LLM rewrite
 * of the file, so evidence can never drift away from its claim. The CAPPING SENTENCE IS
 * LEFT IN PLACE deliberately — it is the record of why the score was wrong, and a reader who
 * finds "held at 6 … at this volume" beside an 8/10 can see that the cap was overruled.
 * Silently deleting it would erase the evidence of the repair.
 */
export function raiseFitScore(content, claim, fromScore, toScore) {
  if (!Number.isInteger(toScore) || toScore < 1 || toScore > 10) {
    throw new Error(`raiseFitScore: toScore must be an integer 1-10, got ${JSON.stringify(toScore)}.`);
  }
  if (toScore <= fromScore) {
    throw new Error(`raiseFitScore: refusing to move ${claim.slice(0, 50)} from ${fromScore} to ${toScore}.`);
  }
  const { head, sections } = splitSections(content);
  const grave = findGraveyard(sections);
  const target = sections.filter((s) => s !== grave).find((s) => s.heading === claim);
  if (!target) throw new Error(`raiseFitScore: no live tactic with the claim "${claim}".`);

  const body = target.lines.join('\n');
  const re = new RegExp(`(\\*\\*Fit here \\()${fromScore}(/10\\):\\*\\*)`);
  if (!re.test(body)) {
    throw new Error(`raiseFitScore: "${claim.slice(0, 50)}" does not carry a ${fromScore}/10 Fit heading.`);
  }
  const rebuilt = sections.map((s) => (s === target ? body.replace(re, `$1${toScore}$2`) : s.lines.join('\n')).trim());
  return [head.trim(), '', ...rebuilt.flatMap((s) => [s, ''])].join('\n');
}

export async function regateAdopted({ items, client, maxTokens = 32000, retryOptions = {}, deadlineMs = STREAM_DEADLINES_MS.regate }) {
  if (!items.length) return [];
  const res = await withRetry(
    () => streamWithDeadline(
      client,
      {
        ...
        max_tokens: maxTokens,
        thinking: { type: 'adaptive' },
        messages: [{ role: 'user', content: buildRegatePrompt({ items }) }],
      },
      { ms: deadlineMs, label: 'regate' },
    ),
    { label: `regate ${items.length}`, ...retryOptions },
  );
  if (res.stop_reason === 'max_tokens') {
    throw new Error(`Re-gate of ${items.length} tactics hit max_tokens — output truncated. Refusing to apply.`);
  }
  const text = (res.content ?? []).filter((b) => b.type === 'text').map((b) => b.text).join('');
  const parsed = parseJsonBlock(text, `the re-gate response for ${items.length} tactics`);

  const decisions = parsed?.decisions;
  if (!Array.isArray(decisions)) throw new Error('Re-gate response has no decisions array.');

  const seen = new Set();
  for (const d of decisions) {
    if (!Number.isInteger(d.index) || d.index < 0 || d.index >= items.length) {
      throw new Error(`Re-gate returned index ${JSON.stringify(d.index)}, outside 0..${items.length - 1}.`);
    }
    if (seen.has(d.index)) throw new Error(`Re-gate returned index ${d.index} twice.`);
    seen.add(d.index);
    if (d.stage != null && !STAGES.includes(d.stage)) {
      throw new Error(`Re-gate returned stage "${d.stage}" for [${d.index}], not one of ${STAGES.join(', ')}.`);
    }
    if (d.stage != null && !d.reasoning) {
      throw new Error(`Re-gate gated [${d.index}] with no reasoning — a gate nobody can justify is not a record.`);
    }
  }
  if (seen.size !== items.length) {
    const missing = items.map((_, i) => i).filter((i) => !seen.has(i));
    throw new Error(`Re-gate skipped ${missing.length} tactic(s): [${missing.slice(0, 10).join(', ')}].`);
  }

  // Positional merge — the index is the join, never the text.
  //
  // BIDIRECTIONAL since 2026-09-06. This used to be `.filter((d) => d.stage != null)`,
  // which silently dropped every "runnable today" verdict and made parking a one-way
  // door. Only CHANGES are returned, so an unchanged tactic still costs no edit:
  //   was unparked, now gated  → action 'park'
  //   was gated, now runnable  → action 'unpark'
  //   gated, but a DIFFERENT gate → action 'repark'
  return decisions
    .map((d) => {
      const item = items[d.index];
      const before = item.stage ?? null;
      const after = d.stage ?? null;
      if (before === after) return null;
      const action = after === null ? 'unpark' : before === null ? 'park' : 'repark';
      return { ...item, stage: after, previousStage: before, action, reasoning: d.reasoning };
    })
    .filter(Boolean);
}

/**
 * Add a `**Stage:**` marker to one live tactic, by text surgery.
 *
 * Deliberately NOT an LLM whole-file rewrite: gating a tactic changes one line, and
 * regenerating the file to add it would risk the evidence-against-claim drift the
 * second-generative-pass rule exists to prevent. Same reasoning as falsifyTactic.
 *
 * The marker goes immediately under the heading, before `**Why it works:**`, matching
 * renderSkillMarkdown byte for byte — extractStagedTactics reads the line right after the
 * heading, so a marker placed anywhere else is invisible to every consumer.
 */
export function insertStageMarker(content, claim, stage) {
  if (!STAGES.includes(stage)) {
    throw new Error(`insertStageMarker: stage must be one of ${STAGES.join(', ')}, got "${stage}".`);
  }
  const { head, sections } = splitSections(content);
  const grave = findGraveyard(sections);
  const live = sections.filter((s) => s !== grave);

  const target = live.find((s) => s.heading === claim);
  if (!target) {
    throw new Error(`insertStageMarker: no live tactic with the claim "${claim}".`);
  }
  if (/^\*\*Stage:\*\*/m.test(target.lines.join('\n'))) {
    throw new Error(`insertStageMarker: "${claim}" already carries a stage.`);
  }

  const lines = [...target.lines];
  // lines[0] is the heading; the renderer always follows it with a blank line.
  const at = lines[1] === '' ? 2 : 1;
  lines.splice(at, 0, `**Stage:** ${stage} — parked until the ${stage} phase opens. Recorded now so it is not re-derived later.`, '');

  const rebuilt = sections.map((s) => (s === target ? lines.join('\n') : s.lines.join('\n')).trim());
  return [head.trim(), '', ...rebuilt.flatMap((s) => [s, ''])].join('\n');
}

/**
 * Remove a `**Stage:**` marker from one live tactic — the inverse of insertStageMarker.
 *
 * WHY THIS HAD TO EXIST: until 2026-09-06 parking was a ONE-WAY DOOR. `regateAdopted`
 * ended with `decisions.filter((d) => d.stage != null)`, so a "runnable today" verdict was
 * computed, paid for, and then discarded — nothing in the agent could ever un-park a
 * tactic. That is fine while the gates only ever tighten, and wrong the moment a
 * constraint turns out to have been mis-stated: the `team` gate listed craft ROLES the
 * operator performs himself, so sound, runnable tactics were parked behind a hire that is
 * not coming, and no amount of re-running could recover them.
 *
 * Same text-surgery discipline as its inverse: one line changes, no LLM rewrite of the
 * file, so evidence can never drift away from its claim.
 */
export function removeStageMarker(content, claim) {
  const { head, sections } = splitSections(content);
  const grave = findGraveyard(sections);
  const live = sections.filter((s) => s !== grave);
  ...
  const target = live.find((s) => s.heading === claim);
  if (!target) {
    throw new Error(`removeStageMarker: no live tactic with the claim "${claim}".`);
  }
  const idx = target.lines.findIndex((l) => /^\*\*Stage:\*\*/.test(l));
  if (idx === -1) {
    throw new Error(`removeStageMarker: "${claim}" carries no stage marker.`);
  }

  const lines = [...target.lines];
  // Drop the marker and the blank line insertStageMarker paired with it, so a
  // park/unpark round trip is byte-identical to never having parked at all.
  const drop = lines[idx + 1] === '' ? 2 : 1;
...
export function assertNewSkillGating({ name, description = '', tactics = [] }) {
  if (!tactics.length) return;
  if (!TEAM_SHAPED.test(`${name} ${description}`)) return;
  if (tactics.some((t) => t.stage)) return;

  const hit = `${name} ${description}`.match(TEAM_SHAPED)[0];
  throw new Error(
    `Refusing to create "${name}": its subject is ${hit}-shaped work, but not one of its ` +
    `${tactics.length} tactics carries a stage. This business is one person at ~54 orders/month ` +
    `on $30/day — work needing people or budget it does not have is "early, not wrong", so it ` +
    `belongs adopted behind the \`team\` or \`scale\` gate rather than live in the fleet ` +
    `projection. Set stage on the tactics that need it, or if they are all genuinely runnable ` +
    `solo today, say so in the fit reasoning and stage at least the ones that are not.`
  );
}
...
export function validateSkillEdit(oldContent, newContent, { supersedes = null } = {}) {
  ...
  if (!supersedes) {
    ...
    if (oldStaged.length) {
      const newStaged = extractStagedTactics(newContent);
      const newClaims = new Set(newStaged.map((t) => t.claim));
      const newNormalized = new Set(newStaged.map((t) => normalizeStagedClaim(t.claim)));
      // Exact match first, then the ordinal-insensitive one. A skill whose headings are a
      // numbered sequence renumbers its own later steps whenever a merge inserts one in
      // the middle, so "Step 5 — X" legitimately becomes "Step 7 — X" with its marker
      // intact — and matching on the raw claim read that as an unpark. That false positive
      // blocked every merge into marketing-performance-pattern-analysis (the fleet's only
      // numbered skill) and killed two ingests before it was diagnosed.
      //
      // It cannot mask a real unpark: the fallback still only looks at tactics that are
      // STAGED in the replacement, so a tactic that lost its marker is absent from both
      // sets however it was renumbered.
      const unparked = oldStaged.filter(
        (t) => !newClaims.has(t.claim) && !newNormalized.has(normalizeStagedClaim(t.claim)),
      );
      if (unparked.length) {
        throw new Error(
 ...
 */
/**
 * Wall-clock ceilings for one streaming call. Generous — these exist to break a hang,
 * not to police a slow-but-working response, and a false fire costs a full retry.
 *
 * Consolidation gets the longest because its output scales with candidate count: 235
 * candidates each carrying a full tactic object is most of a 128k budget, and that
 * legitimately takes many minutes.
 */
export const STREAM_DEADLINES_MS = Object.freeze({
  extraction: 12 * 60_000,
  consolidation: 25 * 60_000,
  merge: 15 * 60_000,
  readjudicate: 15 * 60_000,
  regate: 15 * 60_000,
});

// The deadline itself now lives in lib/stream-deadline.js, because it is not specific to
// this agent: agents/blog-post-writer streams too, and every streaming call site in the
// fleet has the same unbounded window. Re-exported here so existing importers (and the
// tests that already pin this behaviour) keep working.
//
// Failing fast costs almost nothing HERE specifically, which is why this agent can afford
// the strictest possible response: chunk extraction is cached, so the 2026-08-23 re-run
// after a kill served all 16 chunks from cache and only re-paid the one call that stalled.
// NOTE: `export { x } from '...'` creates no LOCAL binding, and this module calls
// streamWithDeadline itself — so import and re-export, don't re-export alone.
import { streamDeadline, streamWithDeadline } from './stream-deadline.js';

export { streamDeadline, streamWithDeadline };
...
/**
 * The claims a skill already holds, as headings. Excludes the Falsified graveyard,
 * which the extraction prompt lists separately under its own reject instruction —
 * listing a dead tactic as if it were live would invite an edit to it.
 */
export function liveTacticHeadings(content) {
  const { sections } = splitSections(content);
  const grave = findGraveyard(sections);
  return sections.filter((s) => s !== grave).map((s) => s.heading);
}

/**
 * What the extractor is shown about skills that already exist — names, descriptions,
 * and the CLAIM HEADINGS, not the full bodies.
 *
 * This is the single projection: buildExtractionPrompt embeds it and the caller hashes
 * it into the chunk cache key. Deriving them separately is how a cache silently starts
 * keying on something the prompt does not contain (the same failure the constraintBlock
 * comment in the caller warns about).
 *
 * Full bodies used to go in here, and it was 94% of the prompt — 144k of 153k tokens on
 * a 25-skill fleet, against ~7k for the excerpt actually being extracted. Two costs, both
 * compounding: every ingest grew the inventory so every later extraction got dearer, and
 * because the fingerprint covered every skill's full text, ANY successful merge
 * invalidated every cached chunk of every source. The cache was correct and useless —
 * measured on 2026-08-23, three consecutive retries of one book re-extracted all six
 * chunks because earlier sources had merged in between.
 *
 * Headings are the right cut because the heading IS the claim, and duplication is judged
 * against claims. A body says why an ALREADY-ADOPTED tactic works, what evidence backs it
 * and how it scored here — none of which tells you whether the tactic now on the table is
 * a restatement. Headings also change far less often than bodies, so the cache can hit.
 */
export function renderInventoryForExtraction(inventory = []) {
  if (!inventory.length) return '(no marketing skills exist yet — every adopted tactic will create one)';
  return inventory
    .map((s) => {
      const claims = liveTacticHeadings(s.content);
      const body = claims.length
        ? claims.map((c) => `- ${c}`).join('\n')
        : '(no tactics recorded yet)';
      return `### ${s.name}\n_${s.description}_\n\n${body}`;
    })
    .join('\n\n---\n\n');
}
...
export function buildExtractionPrompt({ video, inventory = [], chunk = null }) {
  ...
  const inventoryBlock = renderInventoryForExtraction(inventory);
...
Each skill below is listed as its name, its description, and every claim it already
holds. The claims are what to check a candidate tactic against: if the source is
restating one, reject it as a duplicate and name the claim you matched it to. Judge
duplication on the claim, not on wording — a reworded version of a recorded tactic is
still a duplicate.
  ...
  "tactics": [
    {
      ...
      "stage": ${STAGES.map((s) => `"${s}"`).join(' | ')} | null,
...
Rules:
- stage is how you park a tactic per "Timing is a stage, never a reject" above. Set it on an
  ADOPTED tactic that is right for this business but blocked by volume, budget or people;
  set it to null when the tactic is runnable today by one person at ~54 orders/month on
  $30/day. null is a claim that it is runnable NOW — do not use it as the default. Only
  "scale" and "team" are still closed; the four funnel gates opened 2026-08-17.
...
export function validateExtraction(obj) {
  ...
  for (const [i, t] of obj.tactics.entries()) {
    ...
    if (t.verdict === 'reject') {
      ...
      // The KEY is required, `null` is a legal value. An omitted stage and a considered
      // "runnable today" used to be indistinguishable, and that ambiguity is exactly how a
      // missing schema field hid for months: every tactic looked deliberately unparked.
      // Requiring it makes null an assertion someone made rather than a default nobody saw.
      // Safe to demand — measured at 100% emission across 159 adopted tactics / 16
      // independent extraction calls once the schema asked for it — and a stray omission
      // is covered by the single re-sample in extractTactics.
      if (!Object.prototype.hasOwnProperty.call(t, 'stage')) {
        throw new Error(
          `${at}: stage is required on an adopted tactic. Use null to assert it is runnable ` +
          `today by one person at ~54 orders/month on $30/day, or one of ${STAGES.join(', ')}.`
        );
      }
      if (t.stage != null && !STAGES.includes(t.stage)) {
        throw new Error(`${at}: stage must be one of ${STAGES.join(', ')} (or null), got "${t.stage}".`);
      }
 ...
 */
export function parseJsonBlock(text, errorLabel) {
 ...
 */
export async function extractTactics({ video, inventory = [], client, chunk = null, maxTokens = 32000, retryOptions = {}, deadlineMs = STREAM_DEADLINES_MS.extraction }) {
  ...
  const callOnce = () => withRetry(
    () => streamWithDeadline(
      ...
      { ms: deadlineMs, label: 'extraction' },
    ),
...
- targetSkill.description must say WHEN to reach for the skill, starting with "Use when".`;
  ...
  const res = await withRetry(
    () => streamWithDeadline(
      ...
      { ms: STREAM_DEADLINES_MS.readjudicate, label: 'readjudicate' },
    ),
...
Carry \`stage\` through: if ANY candidate you merge carried a stage, the consolidated tactic
keeps one, and where they disagree keep the later gate in this order — ${STAGES.join(' < ')}.
Dropping it here would unpark a tactic that the extraction pass deliberately parked, and
 ...
 */
export async function consolidateTactics({ candidates, source, client, maxTokens = 128000, retryOptions = {}, deadlineMs = STREAM_DEADLINES_MS.consolidation }) {
  ...
  const res = await withRetry(
    () => streamWithDeadline(
      ...
      { ms: deadlineMs, label: 'consolidation' },
    ),
...
{ "content": "<the complete new SKILL.md>", "supersedes": "<what you removed and why, or null>" }`;
  ...
  // max_tokens implies a possible >10-minute operation. finalMessage() gives back the
  // same shape create() returned, so everything below is unchanged.
  const res = await streamWithDeadline(
    client,
    {
      model: EXTRACTION_MODEL,
      max_tokens: maxTokens,
    ...
    { ms: STREAM_DEADLINES_MS.merge, label: `merge ${fm.name}` },
  );
  if (res.stop_reason === 'max_tokens') {

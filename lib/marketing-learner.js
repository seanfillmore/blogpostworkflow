 ...
 *
 * Raised 16k -> 32k -> 64k. The 2026-08-22 raise was deliberately the second-best fix and
 * is on record as such: marketing-conversion-copy-angles hit 45% of the 32k budget (30 live
 * tactics, ~57.8 KB) with four copywriting sources still queued, and the split it is owed
 * is NOT mechanical — 25 of its 30 sections cross-reference each other positionally
 * ("the rule directly below", "the specificity rule above"), so cutting the file silently
 * turns those into dangling pointers. That rewrite is its own change, not something to
 * bolt onto an ingest run. 64k buys the runway to do it deliberately; it does not
 * discharge the debt. Opus 5 caps at 128k output, so one further raise exists and then
 * there is no more road. max_tokens is a ceiling rather

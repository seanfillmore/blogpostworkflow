...
export function validateSkillEdit(oldContent, newContent, { supersedes = null } = {}) {
  ...
  // extractFalsifiedClaims slices from "## Falsified" to EOF and harvests every
  // "### " line after it as a falsified claim. mergeSkillContent hands the model
  // the whole file, so if the model emits a new "## " tactic section BELOW
  // Falsified, that tactic's own "### " subheadings get silently swallowed into
  // the falsified list — live tactics get blocklisted with no error anywhere.
  // Guard the invariant every reader of this file assumes: Falsified is last.
  const { sections: newSections } = splitLiveSections(newContent);
  const falsifiedAt = newSections.findIndex((s) => s.heading === 'Falsified');
  if (falsifiedAt !== -1 && falsifiedAt !== newSections.length - 1) {
    const after = newSections.slice(falsifiedAt + 1).map((s) => `"${s.heading}"`).join(', ');
    throw new Error(
      `Refusing to write "${newFm.name}": "## Falsified" must be the last section in the file, ` +
      `but ${newSections.length - 1 - falsifiedAt} section(s) follow it (${after}). ` +
      `extractFalsifiedClaims reads to EOF from that heading, so anything after it would be ` +
      `misread as a falsified claim and silently blocklisted.`
    );
  }
...
function falsifiedIndex(content) {
  ...
  // [ \t\r] (not just [ \t]): on a CRLF file the line is "## Falsified\r" before
  // the \n, and JS's multiline $ only matches immediately before \n — it does not
  // skip over a trailing \r the way sane regex engines treat \r\n as one newline.
  // Without \r in the class this silently returns -1 on every CRLF skill file,
  // which makes extractFalsifiedClaims return [], which makes validateSkillEdit's
  // oldDead.length check short-circuit — the only non-prompt guard disables itself.
  const match = c.match(/^## Falsified[ \t\r]*$/m);
  if (!match) return -1;
...
export function falsifyTactic(content, { claim, reason, today } = {}) {
  ...
  // Run the body through demoteHeadings so a nested heading inside it (e.g. a
  // tactic whose body contains "### Step 1") cannot masquerade as its own
  // falsified claim — extractFalsifiedClaims reads any "### " line under the
  // Falsified heading, so an un-demoted nested "### Step 1" would show up as a
  // bogus standalone "Step 1" entry in the "Do not propose" list.
...
/**
 * Demote markdown headings in a block (## → ###, ### → ####, etc).
 * Preserve headings inside fenced code blocks (``` or ~~~) unchanged.
 *
 * H1 (`# Title`) is demoted by TWO levels, not one: a skill's own H1 is its
 * title, and renderContextMirror already wraps each skill in its own `##
 * marketing-*` section header. Demoting H1 by only one level (to ##) collides
 * with that wrapper — two adjacent ## headings with no nesting relationship.
 * Demoting it to ### nests it correctly under its own skill section.
 */
function demoteHeadings(text) {
  ...
  for (const line of lines) {
    // Track fence state: ``` or ~~~ toggles in/out
    if (line.startsWith('```') || line.startsWith('~~~')) {
      inFence = !inFence;
    }
    const m = !inFence && line.match(/^(#+)[ \t]/);
    if (m) {
      const extra = m[1].length === 1 ? '##' : '#';
      result.push(extra + line);
    } else {
      result.push(line);
    }
  }
...
export function renderContextMirror(inventory = []) {
  ...
  for (const s of inventory) {
    ...
    // Demote headings in the body so they nest logically under the skill section
    const demoted = demoteHeadings(fm.body.trim());

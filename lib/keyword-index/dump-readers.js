...
/**
 * Turn `readUntappedCandidates`'s raw candidate list into the normalized Map
 * `mergeSources` expects as its `untapped` argument.
 *
 * Every keyword is run through `normalize()` before it becomes a Map key —
 * `mergeSources` keys everything else on the same normalized form, and a raw
 * GSC query string (mixed case, stray punctuation) would never match. Skipping
 * this step is a silent failure: every lookup misses, the build still
 * "succeeds," and zero untapped keywords are admitted.
 */
export function buildUntappedMap(candidates) {
  const m = new Map();
  for (const c of candidates || []) {

 ...
 *
 * gsc_untapped entries (queries Google shows us that nobody clicks) are
 * excluded — they may drive optimization of existing pages but must never
 * commission new content while the Traffic phase gates top-of-funnel discovery.
 *
 ...
 * (purchases > 0 AND conversion_share > 0), sorted by their product
 * (purchases × conversion_share) desc — best-ROI commercial keywords first.
 *
 * Used by ads-optimizer to surface high-validation `keyword_add` candidates.
 */
export function topAmazonValidatedForAds(index, { limit = 20 } = {}) {
  ...
  for (const e of Object.values(index.keywords)) {
    const a = e?.amazon;
    if (!a) continue;
    if ((a.purchases ?? 0) <= 0) continue;
    if ((a.conversion_share ?? 0) <= 0) continue;
    out.push(e);
  }
  out.sort((x, y) => {
    const xs = (x.amazon.purchases ?? 0) * (x.amazon.conversion_share ?? 0);
    const ys = (y.amazon.purchases ?? 0) * (y.amazon.conversion_share ?? 0);
    return ys - xs;
  });
  return out.slice(0, limit);
}

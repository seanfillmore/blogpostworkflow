...
export function aggregateGscWindow({ snapshotsDir, fromDate, toDate }) {
  ...
  for (const file of files) {
    ...
    // Real GSC snapshots store the (query, page) join under `queriesByPage`.
    // Older fixtures and prototype snapshots used `queries`; accept both.
    const rows = snap.queriesByPage || snap.queries || [];

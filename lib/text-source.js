...
/**
 * What the provenance line calls this source: `*Source: Creator — "Title" (book, part 7 of 11)*`.
 *
 * Free-form rather than an enum, because the enum is always missing the thing
 * you actually have — but narrow, because the value is interpolated into a
 * parenthesised provenance line that the merge prompt orders the model to copy
 * verbatim. A stray `)` or newline would corrupt every line it touches.
 */
const SOURCE_KIND_RE = /^[a-z][a-z0-9 -]{1,23}$/;

export function validateSourceKind(kind) {
  if (!SOURCE_KIND_RE.test(String(kind ?? ''))) {
    throw new TextSourceError(
      `--source-kind "${kind ?? ''}" must be 2-24 lowercase characters (letters, digits, ` +
      `spaces, hyphens) — e.g. book, essay, social post, newsletter, transcript.`,
      { code: 'BAD_SOURCE_KIND' },
    );
  }
  return kind;
}

export function loadTextFile(path, { author, title, publishedAt = null, sourceKind = 'book' } = {}) {
  ...
  return {
    ...
    sourceType: 'file',
    // sourceType is which loader produced this; sourceKind is what the work IS.
    // Only the second one belongs in a provenance line a human will read.
    sourceKind,
    videoId: null,
    title,
    creator: author,

 ...
 *
 * Resolution order:
 *   1. Exact: a post dir named for the handle (with meta.json) exists. For an
 *      input known to be an ARTICLE handle, use `resolveArticleHandle`, which
 *      does not trust a colliding dir name.
...
export function resolvePostSlug(urlOrHandle) {
  ...
  return resolveSlugAmong(handle, allPostMetas());
}

/**
 * Resolve a SHOPIFY ARTICLE HANDLE — never a post slug — to the post that holds
 * that article.
 *
 * `resolvePostSlug` takes either, so it must let a directory name win: callers
 * such as the winner lock pass post slugs, and `cocoa-butter-lotion/` is that
 * post even though its meta declares the longer handle. Measured against the
 * 2026-09-21 production corpus, dropping the dir-name match globally changed 161
 * lookups and flipped 38 lock verdicts.
 *
 * But for an input known to be an article handle, the dir name is the WRONG
 * identity whenever it collides. 18 of 221 production dirs are named for one
 * handle while their meta records ANOTHER article — typically the `<handle>-2`
 * dedup duplicate. `organic-coconut-oil-types-uses-benefits-for-skin/` holds
 * article `…-2`, while article `organic-coconut-oil-types-uses-benefits-for-skin`
 * lives in `what-is-organic-coconut-oil-uses-benefits-types/`. Resolving that
 * handle by dir name made content-refresher write article A's refresh into
 * article B's directory — where a later publish would have pushed it onto B.
 * Here a post's DECLARED handle wins, and a dir declaring another article is
 * never matched on its name.
 */
export function resolveArticleHandle(urlOrHandle) {
  const handle = handleFromUrl(urlOrHandle);
  if (!handle) return null;
  return resolveSlugAmong(handle, allPostMetas(), { declaredWins: true });
}

function allPostMetas() {
  return listAllSlugs().map((slug) => [slug, getPostMeta(slug)]).filter(([, m]) => m);
}

/** Every Shopify handle a post's meta declares for itself. */
export function declaredHandles(meta) {
  if (!meta) return [];
  return [meta.handle, handleFromUrl(meta.url), meta.shopify_handle, handleFromUrl(meta.shopify_url)]
    .filter(Boolean);
}

/**
 * True when a post's meta names its Shopify article and that article is NOT
 * `handle`. A meta that declares nothing is not "other" — the legacy/bootstrap
 * shape where the directory name is the only identity there is.
 */
export function declaresOtherArticle(meta, handle) {
  const declared = declaredHandles(meta);
  return declared.length > 0 && !declared.includes(handle);
}

/**
 * The pure core of both resolvers: resolve `handle` among `[slug, meta]` pairs.
 * `declaredWins` is `resolveArticleHandle`'s rule (see there). Exported so the
 * resolution order is testable without a filesystem.
 */
export function resolveSlugAmong(handle, metas, { declaredWins = false } = {}) {
  const otherArticle = (meta) => declaredWins && declaresOtherArticle(meta, handle);
  const exactDir = () => {
    const exact = metas.find(([s]) => s === handle);
    return exact && !otherArticle(exact[1]) ? handle : null;
  };
  const declared = () => {
  ...
  // 1. Exact dir match — AFTER the declared match for an article handle: four
  //    production dirs are named for a handle, declare nothing and hold no
  //    article id (leftovers), while a published post declares that handle.
  const hit = declaredWins ? (declared() || exactDir()) : (exactDir() || declared());
  if (hit) return hit;

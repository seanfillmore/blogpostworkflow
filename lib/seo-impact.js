...
import { SEARCH_HOSTS, isSearchHost } from './order-attribution.js';
...
export function isSearchEngineSource(source) {
  ...
  const s = String(source).trim().toLowerCase()
    ...
    .replace(/^www\./, '');
  if (!s) return false;
  // isSearchHost covers search-family subdomains (GA4 reported `noai.duckduckgo.com`
  // as a Referral source on 2026-09-09) with the same rule the order side uses.
  return searchTokens().has(s) || isSearchHost(s);
}
...
/**
 * Entry-page organic revenue per cluster, straight from a `shopifyRevenueByPage`
 * map — no GSC, no page-impact join.
 *
 * `clusterRollup` below answers the same question for the report's own window,
 * but only as a by-product of the impact rows, which exist only where GSC has
 * data for that window. The 90-day judging window has no GSC pull behind it and
 * does not need one: the landing path is on the ORDER, so the revenue side alone
 * is computable from the wide order rows already fetched for the trend.
 *
 * This is SOURCE B of `lib/cluster-hold.js`'s two-source rule — what a category's
 * PAGES earned, keyed on the landing-page URL. Deliberately a separate function
 * from `clusterRollup` rather than a flag on it: this one has no clicks, no page
 * count and no prior-window comparison, and a shared shape that silently omitted
 * three of five fields would read as data loss.
 *
 * @param {Map<string,{revenue:number}>} byPage  from `shopifyRevenueByPage`
 * @param {(path:string)=>string|null} clusterFor
 * @returns {{byCluster:Record<string,number>, residual:number}}
 */
export function entryPageClusterRevenue(byPage, clusterFor) {
  const byCluster = {};
  let residual = 0;
  for (const [path, v] of (byPage?.entries?.() || [])) {
    const revenue = Number(v?.revenue) || 0;
    const cluster = clusterFor(path);
    // Same rule as `residualRollup`: a path matching no cluster is not a
    // category and never becomes a row. It is reported as the residual so the
    // table can be seen not to reconcile, rather than quietly not reconciling.
    if (!cluster) { residual = round2(residual + revenue); continue; }
    byCluster[cluster] = round2((byCluster[cluster] || 0) + revenue);
  }
  return { byCluster, residual };
}

/**
 * Merge source B onto the wide product rows, so one array carries both sources
 * for the same window.
 *
 * UNIONING, not decorating. A cluster whose PAGES earned while its PRODUCTS sold
 * nothing gets a row it would otherwise not have — and that row is the single
 * most important one in the array, because it is exactly the case that must
 * block a hold. A category with no SKU behind it (`coconut oil`: RSC ships no
 * coconut-oil product) can never appear in `productClusterRows` at all, so
 * decorating instead of unioning would make it invisible to source B and leave
 * the hold to be decided by source A alone.
 *
 * A NULL SOURCE B YIELDS AN EMPTY ARRAY, and that guard is here rather than only
 * at the call site on purpose. Stamping `entry_page_organic_revenue: 0` on every
 * row when B was never measured is the "read a missing field as `|| 0`" mistake
 * that `lib/cluster-hold.js`'s `readWideSources` exists to refuse — except here
 * it would be baked into the report on disk, where the reader has no way left to
 * tell it apart from a genuine zero. Emitting nothing switches the $0-cluster
 * gate off for that run; emitting half of it tells the gate that every cluster's
 * pages earned nothing, which is the direction that CREATES holds.
 *
 * @param {Array<object>} productRows  from `productClusterRows`
 * @param {{byCluster:Record<string,number>}|null} wideEntryPage
 */
export function withWideEntryPageRevenue(productRows, wideEntryPage) {
  if (!wideEntryPage?.byCluster) return [];
  const earned = wideEntryPage.byCluster;
  const byCluster = new Map();
  for (const r of productRows || []) {
    byCluster.set(r.cluster, { ...r, entry_page_organic_revenue: round2(earned[r.cluster] || 0) });
  }
  for (const [cluster, revenue] of Object.entries(earned)) {
    if (byCluster.has(cluster)) continue;
    byCluster.set(cluster, {
      cluster,
      product_organic_revenue: 0,
      product_organic_orders: 0,
      product_revenue_all_channels: 0,
      product_orders_all_channels: 0,
      organic_share: null,
      entry_page_organic_revenue: round2(revenue),
    });
  }
 ...
 *
 * The money here is ENTRY-PAGE-CREDITED, ORGANIC-SEARCH-ONLY revenue over this
 * report's window, bucketed by a word in the landing page's URL. It is not
 * product revenue and it does not reconcile to a category's sales — see the
 * header of `lib/cluster-revenue.js`. `entry_page_organic_revenue` is the
 * canonical field name; `revenue` is emitted alongside it as a back-compatible
 * alias holding exactly the same number, for consumers not yet migrated.
 */
export function clusterRollup(impacts, clusterFor) {
  ...
  const out = [...m.values()].map((c) => ({
    ...c,
    revenue: round2(c.revenue),
    entry_page_organic_revenue: round2(c.revenue),
...
/**
 * Everything `clusterRollup` dropped on the floor — the RESIDUAL.
 *
 * `clusterRollup` skips any page whose URL matches no cluster, so the cluster
 * table has never summed to the report's own `totals.organic_revenue` and has
 * never said so. On the 2026-08-22 report that gap is $77.94 (the homepage);
 * over 90 days it is $1,152.50, 54% of the store, landing on `/`, `/cart` and
 * subscription pages. An invisible majority is exactly what makes the table look
 * like it ought to reconcile to product revenue — which is the misreading that
 * let a $430 category be recorded as $0.
 *
 * Returned as its own object rather than as a cluster row on purpose: it is not
 * a category, must never be classified as one, and must never be eligible for a
 * `proven_dud` verdict.
 */
export function residualRollup(impacts, clusterFor) {
  const pages = [];
  let revenue = 0; let revenuePrev = 0; let clicks = 0;
  for (const i of impacts || []) {
    if (clusterFor(i.path)) continue;
    revenue += i.revenue || 0;
    revenuePrev += i.revenuePrev || 0;
    clicks += i.clicks || 0;
    if ((i.revenue || 0) > 0) pages.push({ path: i.path, revenue: round2(i.revenue) });
  }
  return {
    label: 'no cluster (homepage / cart / subscription)',
    entry_page_organic_revenue: round2(revenue),
    revenue: round2(revenue),
    revenuePrev: round2(revenuePrev),
    revenueDelta: round2(revenue - revenuePrev),
    clicks,
    pages: (impacts || []).filter((i) => !clusterFor(i.path)).length,
    top_pages: pages.sort((a, b) => b.revenue - a.revenue).slice(0, 5),
  };
}

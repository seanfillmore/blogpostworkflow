...
import { orderLines } from './product-cluster-revenue.js';
...
export const SEARCH_HOSTS = new Set([
  ...
  'baidu.com', 'qwant.com', 'mojeek.com',
]);

// Registrable domains where EVERY subdomain is a search surface, so a referrer from
// any of them is an unpaid search click. DuckDuckGo serves its no-AI results from
// `noai.duckduckgo.com` (and has `html.` / `lite.` / `start.` variants); exact-host
// matching filed order #2350 (2026-09-10, a blog-entry toothpaste order) as
// `referral` and dropped it out of organic revenue.
//
// Deliberately NOT google.com: across 400 days of orders its subdomains include
// `mail.google.com` (email, 2 orders) and `tagassistant.google.com` (debugging,
// 2 orders), and `gemini.google.com` is an AI assistant. Suffix-matching Google
// would relabel all of them organic search. Add a family here only when no
// subdomain of it can be anything but search.
export const SEARCH_HOST_FAMILIES = new Set(['duckduckgo.com']);

/** True when a bare hostname is an unpaid search engine (exact host or a search family subdomain). */
export function isSearchHost(host) {
  if (!host) return false;
  const h = String(host).toLowerCase();
  if (SEARCH_HOSTS.has(h)) return true;
  for (const family of SEARCH_HOST_FAMILIES) {
    if (h.endsWith(`.${family}`)) return true;
  }
  return false;
}

// AI assistants are a SEPARATE earned channel, never search. These hosts were
// originally folded into SEARCH_HOSTS, which booked every AI-referred order as
// `organic-search` — $383.61 of AI revenue credited to SEO — and, once
// seo-impact began importing SEARCH_HOSTS to rescue sessions GA4 misfiles as
// Referral, rescued AI sessions as organic ones too. Both halves of the SEO
// number moved the same way, so the error was invisible in a self-consistency
// check. They behave nothing alike: the AI revenue here came from a ChatGPT
// shopping product card (a feed surface), not from a cited page.
export const AI_ASSISTANT_HOSTS = new Set([
  'chatgpt.com', 'chat.openai.com', 'openai.com', 'claude.ai', 'perplexity.ai',
  'gemini.google.com', 'bard.google.com', 'copilot.microsoft.com', 'you.com', 'poe.com',
]);
...
/**
 * Host named by `utm_source`, normalized the same way a referrer host is.
 *
 * Needed because the referrer is the LESS reliable signal on these orders: it
 * routinely survives from an earlier tab (an order tagged `utm_source=chatgpt.com`
 * arrived with `swansonvitamins.com` as its referrer) or is stripped entirely.
 * `utm_source` is set by the sending surface and does not drift, so it is
 * consulted first for channel identity.
 *
 * A bare token like `chatgpt` is accepted as well as `chatgpt.com`, since
 * feeds tag both ways.
 */
function utmSourceHost(landingSite) {
  const q = queryOf(landingSite);
  const raw = String(q?.get('utm_source') || '').trim().toLowerCase();
  if (!raw) return null;
  const host = raw.replace(/^https?:\/\//, '').split('/')[0].replace(/^www\./, '');
  if (!host) return null;
  if (SEARCH_HOSTS.has(host) || AI_ASSISTANT_HOSTS.has(host)) return host;
  // bare token → match against the registrable prefix of a known host
  for (const known of [...AI_ASSISTANT_HOSTS, ...SEARCH_HOSTS]) {
    if (known.split('.')[0] === host) return known;
  }
  return host;
}
...
/**
 * Landing-page type from a path. Exported so lib/commercial-cvr.js can bucket GA4
 * landing pages by the SAME vocabulary this file buckets orders by — a second copy
 * would let the sessions denominator and the orders numerator drift apart, which is
 * exactly the kind of split that produced a wrong cluster verdict in cluster-revenue.
 */
...
/**
 * ISO-2 country code from the order's addresses, shipping first.
 *
 * A country CODE is coarse and non-identifying — it is `US`, not a street — so it
 * does not belong with the `sku` and `properties` fields classifyOrder's record
 * comment deliberately excludes from these offsite-backed snapshots.
 *
 * Carried because lib/commercial-cvr.js excludes non-US SESSIONS from the CVR
 * denominator, and doing that without a matching filter on the numerator would
 * make the rate read too HIGH. Returns null rather than guessing: an unknown
 * country must never be read as non-US, because dropping a real order from the
 * numerator makes the rate read too low.
 */
function countryCodeOf(order) {
  const raw = order?.shipping_address?.country_code || order?.billing_address?.country_code || null;
  if (!raw) return null;
...
export function classifyOrder(order) {
  ...
  const channel = (() => {
    ...
    // Paid outranks source attribution: a gclid-carrying order must never be
    // laundered into an earned channel by whatever utm_source rode along with it.
    if (paid) return 'paid-search';
    // utm_source before referrer — see utmSourceHost() for why the referrer is
    // the weaker signal on exactly these orders.
    const srcHost = utmSourceHost(order.landing_site);
    if (srcHost && AI_ASSISTANT_HOSTS.has(srcHost)) return 'ai-assistant';
    if (srcHost && isSearchHost(srcHost)) return 'organic-search';
    if (referrerHost && AI_ASSISTANT_HOSTS.has(referrerHost)) return 'ai-assistant';
    if (referrerHost && isSearchHost(referrerHost)) return 'organic-search';
  ...
  return {
    ...
    referrerHost,
    // ISO-2, or null. See countryCodeOf() — additive, and never PII.
    country: countryCodeOf(order),
    channel,
    paid,
    clickId,
    isTest,
    ...
    // What was in the order. TITLE, PRODUCT ID AND LINE REVENUE ONLY — these
    // records are backed up offsite, so no sku (which can carry a customer
    // reference), and never `properties`, which is free text a buyer typed.
    //
    // Carried because the ENTRY PAGE cannot answer what a CATEGORY sold: 29% of
    // trailing-90-day organic revenue lands on `/`, `/cart/...` or a
    // subscription order with no landing page at all, and a soap bought in a
    // cart that entered on the homepage is credited to the homepage. See
    // lib/product-cluster-revenue.js, which rolls these up per cluster.

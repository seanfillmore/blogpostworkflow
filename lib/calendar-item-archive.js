...
//
// WHY THE CALENDAR IS NOT WRITTEN THROUGH `lib/calendar-store.js`. Its
// `writeCalendar` NORMALISES every item onto a fixed 18-field schema, which
// silently drops anything outside it — including `possible_duplicate`,
// `ranked_match` and `duplicate_of`, the very flags PR #921 wired into
// `agents/pipeline-prioritizer` and `agents/content-strategist`. A sweep whose
// job is to remove duplicates must not strip the duplicate evidence off every
// item it KEEPS, so `writeCalendarItems` below preserves each surviving item
// byte for byte and touches nothing but the membership of `items[]`.

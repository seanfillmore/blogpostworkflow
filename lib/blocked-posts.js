...
import { resolvePublishStatus, isRetiredPost } from './post-publish-state.js';
 ...
 * and failed, on exactly this verdict?" — see rule 7. Deliberately content-based
 ...
 *   2. A RETIRED post is never blocked, in EITHER audience. A page that was
 *      consolidated away, unpublished or archived on purpose has no reader to
 *      serve and no ranking to protect, so every action this verdict offers —
 *      Fix blockers, Re-run editor, Kill article — is effort spent on a URL
 *      that 301s. Deliberately BEFORE the `includeLive` split: a retired page is
 *      neither a live page the resolver owns nor pre-publish work a human owes,
 *      so no audience wants the row. See `RETIRED_STATUSES`.
 *   3. A SCHEDULED post is never blocked: it goes live on its own.
 *   4. "## OVERALL QUALITY" (the editor's canonical sign-off) trumps sub-section
 *      verdicts: Pass / Good / Excellent → not blocked.
 *   5. "## BLOCKERS*" starting with "None" → not blocked.
 *   6. A LIVE post is blocked only while its failing report is FRESH, and only
 ...
 *   7. A post already exhausted by blocked-post-resolver against THIS EXACT
...
export function classifyBlockedReport({
  ...
  if (isRetiredPost(meta, { now })) return null;                                           // rule 2

  const status = resolvePublishStatus(meta, { now });
  if (status === 'scheduled') return null;                                                 // rule 3

  const overallMatch = report.match(/##[^\n]*OVERALL QUALITY[^\n]*\n[\s\S]*?VERDICT[:*\s]+([^\n]+)/i);
  if (overallMatch && !/needs work/i.test(overallMatch[1])) return null;                   // rule 4

  const blockersMatch = report.match(/##[^\n]*BLOCKER[^\n]*\n([\s\S]*?)(?=\n##|\n---|$)/i);
  if (blockersMatch && /^\s*None\b/i.test(blockersMatch[1].trim())) return null;           // rule 5

  const live = status === 'published';
  if (live && (!includeLive || reportAgeDays > LIVE_BLOCK_FRESHNESS_DAYS)) return null;    // rule 6

  const prior = meta.blocked_resolution;
  if (prior && prior.report_fingerprint === reportFingerprint(report)) return null;        // rule 7

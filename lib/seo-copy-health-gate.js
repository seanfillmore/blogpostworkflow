...
//
// ── ORAL CARE AND CHILD SAFETY, added 2026-09-13 ─────────────────────────────────
//
// Toothpaste orders began arriving through blog posts in September 2026, and the copy
// beside those buy boxes said "Support natural remineralization with Real Skin Care's…"
// and "Real Skin Care's formula is safe for kids". Nothing here caught either: no word in
// them was in the vocabulary. scripts/remediate-toothpaste-safety-claims.mjs cleaned the
// live corpus; this is what stops the next generation writing it again.
//
// TWO additions, living in two different places ON PURPOSE:
//
//   1. DENTAL CONDITIONS (disease) and ENAMEL-REPAIR / ANTICAVITY language
//      (`oral-drug-claim`) go into HEALTH_CLAIM_PATTERNS — shared with the ad gate, the
//      PDP builder and persona screening — because an anticaries claim is a drug claim
//      on every surface. Dental conditions inherit the existing surface rule: an article
//      may name cavities, product copy may not, and "prevents cavities" blocks anywhere.
//      `oral-drug-claim` blocks everywhere, like the therapeutic verbs.
//
//   2. CHILD SAFETY ("safe for kids", "safe to swallow", "kid-safe") lives ONLY HERE, and
//      that is a measurement, not a preference. data/context/personas.json carries it
//      twice — a persona SUMMARY ("kid-safe hand soap") and an objection quote ("Is this
//      actually safe for kids…?") — and `sanitizePersonas` drops a whole persona when its
//      summary violates. Putting it in the shared list would have cost a live persona and
//      its angles for a word that is a substantiation question, not a drug claim.
//      Tier: COMMERCIAL blocks (the product speaking); EDITORIAL blocks only when the
//      brand is named in the same string ("Safe for kids and adults, Real Skin Care's
//      formula…" was a live description_tag); otherwise it is advisory, because "Is
//      Natural Deodorant Safe for Kids?" is a legitimate article a page ranks for.
//
// MEASURED before shipping, read-only, 2026-09-13:
//   · 1,251 gated live strings (article titles, summaries, title_tag/description_tag;
//     product titles, bodies and metafields; pages; local meta.json; A/B proposals;
//     queue copy): 0 hits for any of the new vocabulary.
//   · 90 collections / 338 strings: 12 hits, 11 of them on DRAFTS. The one live hit is a
//     foaming-hand-soap FAQ answering "Is this soap safe for kids…?" with "Yes." — a
//     product child-safety claim this gate is right to refuse on regeneration.
//   · Search Console, 28 days: queries containing the words drew 7 (dental), 31
//     (remineralization) and 27 (child safety) impressions and 0 clicks. No page's own
//     ranking query needs them, so no CTR rewrite is stranded — the failure that keeps
//     `toxicity` advisory.
//   · Live theme (the daily copy-surface detector's corpus): the new vocabulary appears
//     only in customer-question FAQ HEADINGS, a founder story about CONVENTIONAL paste,
//     and a sentence naming an ingredient we do NOT use — all acknowledged in
//     lib/theme-claim-keeps.js with their reasons.
//
...
export const DISEASE_NAMED_CATEGORY = 'disease-named';

/**
 * Child-safety language. CONDITIONAL, like `disease`: blocking as `child-safety` when the
 * product is speaking, advisory as `child-safety-mentioned` when an article merely asks.
 * See the header's oral-care section for why this is not in HEALTH_CLAIM_PATTERNS.
 */
export const CHILD_SAFETY_CATEGORY = 'child-safety';
export const CHILD_SAFETY_MENTIONED_CATEGORY = 'child-safety-mentioned';

/**
 * WHICH SURFACE IS SPEAKING.
 *
 * `EDITORIAL` is an article talking about the world — it may name a skin
 * condition, because that is the topic. `COMMERCIAL` is the product speaking:
 * a product title or body, a collection description, a homepage card above a
 * buy link. Naming a condition there positions the cosmetic FOR that condition,
 * which is an intended-use claim with no claim vocabulary in it — the same
 * shape `lib/product-category-terms.js` blocks for "our antiperspirant".
 *
 * **COMMERCIAL is the DEFAULT, deliberately.** A caller that forgets to declare
 * itself gets the strict answer. The cost of being wrong in that direction is a
 * regeneration; the cost of the other direction is an unapproved-drug claim on a
 * page with a buy button, which is what `scripts/remediate-homepage-card-claims.mjs`
 * already had to clean up once ("For dry patches, eczema, and overnight repair."
 * directly above a Shop link).
 */
export const EDITORIAL_SURFACE = 'editorial';
export const COMMERCIAL_SURFACE = 'commercial';

export const ADVISORY_CATEGORIES = new Set([
  DISEASE_NAMED_CATEGORY,
  CHILD_SAFETY_MENTIONED_CATEGORY,
  // `disease` is NOT here: it is CONDITIONAL — advisory when a condition is
  // merely named, blocking when the copy claims to fix it. See CURE_CONTEXT.
  'toxicity',
  'therapeutic-noun',
 ...
 */
/**
 * The disease vocabulary, taken from the SHARED pattern rather than restated —
 * same single-source rule as HEALTH_CLAIM_PATTERNS itself. Used only to decide
 * whether a therapeutic NOUN beside it is still informational.
 */
const DISEASE_PATTERN = new RegExp(
  HEALTH_CLAIM_PATTERNS.find((p) => p.category === 'disease').pattern.source, 'i',
);

/**
 * VERBS THAT TURN NAMING A CONDITION INTO CLAIMING TO FIX IT.
 *
 * Deliberately WIDER than the `therapeutic` pattern, and deliberately only
 * consulted when a condition is named in the same string. That split is the
 * whole design:
 *
 *   "fights odor"    → fine, and is real deodorant copy this brand ships
 *   "fights eczema"  → a treatment claim
 *
 * `therapeutic` alone is not enough — it holds heal/cure/treat/prevent but NOT
 * "clears up", "gets rid of", "fights" or "eliminates", so
 * "Clears Up Psoriasis Fast" passed every gate while `disease` was advisory.
 * That gap is exactly why this list exists, and why it may never be promoted to
 * an unconditional block: half of it is ordinary cosmetic language away from a
 * disease name.
 */
const CURE_CONTEXT = /\b(heals?|healed|healing|cures?|cured|curing|treats?|treated|treating|treatment|remed(?:y|ies)|prevents?|prevented|preventing|prevention|reverses?|reversing|clears?\s+up|cleared\s+up|clearing\s+up|gets?\s+rid\s+of|fights?|fighting|combats?|eliminates?|eliminating|banish(?:es|ing)?|stops?|relieves?|relieving|soothes?\s+away|fix(?:es|ing)?|calms?\s+down|end(?:s|ing)?\s+the)\b/i;

/**
 * A safety assertion about children or ingestion. Matched here rather than in
 * HEALTH_CLAIM_PATTERNS — see the header. "safer for kids" and "kid-safe" are the same
 * assertion; "supervise young kids" and "ask your pediatric dentist" are not matched and
 * are exactly the replacement wording this gate's retry asks for.
 */
const CHILD_SAFETY_PATTERN = /\b(?:safe|safer|safest)\s+(?:for|with|around)\s+(?:kids|children|toddlers|babies|infants|little\s+ones)\b|\bsafe\s+(?:if|when)\s+swallowed\b|\bsafe\s+to\s+swallow\b|\b(?:kid|child|baby|toddler)[-\s]safe\b/i;

/**
 * The brand speaking about its own products, in the same string. Only consulted for
 * child-safety on an EDITORIAL surface: a question about a category is an article topic,
 * the same words next to "Real Skin Care's formula" are a claim about ours.
 */
const BRAND_CONTEXT = /\b(?:real\s+skin\s+care|our\s+(?:formulas?|toothpastes?|deodorants?|lotions?|soaps?|creams?|balms?|products?))\b/i;

/**
 * The standing instruction for oral care and child safety, stated in BOTH directions for
 * the same reason REGULATORY_PHRASING_RULE is: a model told only "never mention cavities"
 * strips the fluoride explanation that fluoride-free toothpaste pages exist to give.
 */
export const ORAL_CARE_COMPLIANCE_RULE =
  'ORAL CARE AND CHILD SAFETY (hard rule): our toothpaste is a fluoride-free COSMETIC. Say it '
  + 'cleans, polishes, freshens breath and is gentle. Never say it remineralizes, rebuilds, repairs '
  + 'or strengthens enamel, or prevents, fights or protects against cavities, tooth decay or gum '
  + 'disease — those are drug claims. You MAY explain what fluoride or hydroxyapatite does as a '
  + 'CATEGORY, and an article may discuss cavities. Never say our products are safe for kids or safe '
  + 'to swallow; say to ask a pediatric dentist and to supervise young children. An article may ask '
  + 'whether a product category is safe for kids.';
...
const DEMOTED = {
  therapeutic: {
    ...
    // THE ONE LEAK THAT OPENED WHEN `disease` BECAME ADVISORY, closed here.
    // "remedy" is advisory and, since 2026-09-07, so is "eczema" — so
    // "eczema remedy" would have been fully advisory and shipped, which is
    // precisely the cure-talk the operator's rule forbids. A therapeutic NOUN
    // beside a named condition is a claim about that condition, so the demotion
    // does not apply when one is present.
    notWhen: DISEASE_PATTERN,
    notWhenWhy: 'names a remedy FOR a named condition — that is a claim to treat it, not an informational reference',
  },
};
 ...
 */
export function findSeoCopyClaims(text, surface = COMMERCIAL_SURFACE) {
  const s = String(text ?? '');
  const blocking = [];
  const advisory = [];
  ...
  for (const { category, why, pattern } of HEALTH_CLAIM_PATTERNS) {
    ...
    for (const m of s.matchAll(new RegExp(pattern.source, 'gi'))) {
      ...
      if (demotion && demotion.pattern.test(match)) {
        // A demotion can be withdrawn by context: see `notWhen`.
        if (demotion.notWhen && demotion.notWhen.test(s)) {
          blocking.push({ category, why: demotion.notWhenWhy, match });
        } else {
      ...
      // A NAMED CONDITION is advisory on its own and blocking in a cure context.
      // Operator rule, 2026-09-07: "we shouldn't outright reject eczema as a
      // topic, our products can help; we just need to limit the conversation to
      // prevent it talking about curing." Measured: of 447 live sentences naming
      // a condition, 406 (91%) were editorial or positional.
      if (category === 'disease') {
        // THREE-WAY, decided by the SURFACE as well as the words:
        //   cure verb, anywhere            -> BLOCKING
        //   named condition, EDITORIAL     -> advisory   (an article may discuss it)
        //   named condition, COMMERCIAL    -> BLOCKING   (the product speaking)
        if (CURE_CONTEXT.test(s) || surface !== EDITORIAL_SURFACE) {
          blocking.push({
            category,
            why: CURE_CONTEXT.test(s)
              ? 'claims to fix a named condition — a cosmetic that treats a disease is an unapproved drug. Naming the condition is fine; claiming to fix it is not'
              : 'names a condition in COMMERCIAL copy — positioning a cosmetic for a disease is an intended-use claim even with no claim vocabulary. Editorial copy may discuss the condition; product copy may not be sold for it',
            match,
          });
        } else {
          advisory.push({
            category: DISEASE_NAMED_CATEGORY,
            why: 'names a skin condition without claiming to fix it — reported, not blocked',
            match,
          });
        }
        continue;
  ...
  // CHILD SAFETY — conditional on the surface and, editorially, on the brand speaking.
  for (const m of s.matchAll(new RegExp(CHILD_SAFETY_PATTERN.source, 'gi'))) {
    const match = m[0];
    if (surface !== EDITORIAL_SURFACE) {
      blocking.push({
        category: CHILD_SAFETY_CATEGORY,
        why: 'asserts child or ingestion safety in COMMERCIAL copy — an unsubstantiated safety claim about a product that contains essential oils. Say to ask a pediatric dentist and to supervise young children instead',
        match,
      });
    } else if (BRAND_CONTEXT.test(s)) {
      blocking.push({
        category: CHILD_SAFETY_CATEGORY,
        why: 'asserts child or ingestion safety beside the brand — a claim about OUR product, not a question about the category. An article may ask whether a category is safe for kids; it may not say our formula is',
        match,
      });
    } else {
      advisory.push({
        category: CHILD_SAFETY_MENTIONED_CATEGORY,
        why: 'discusses child safety as a category question — reported, not blocked',
        match,
      });
    }
  }
 ...
 */
export function checkSeoCopyFields(fields, { surface = COMMERCIAL_SURFACE } = {}) {
  const blocking = [];
  const advisory = [];
  for (const [field, value] of Object.entries(fields || {})) {
    const hits = findSeoCopyClaims(plainText(value), surface);
 ...
 *             advisory: Array<{field:string, category:string, why:string, match:string}>}}
 */
export function checkSeoCopy(copy, opts) {
  return checkSeoCopyFields({ title: copy?.title, meta: copy?.meta }, opts);
}
...
export function seoCopyConstraint(violations) {
  ...
  const lines = [
    ...
    'Do not say it heals, cures, treats, remedies or prevents anything, do not name a',
    'medication, and do not claim clinical or regulatory backing. Rewrite without those words.',
    'You MAY name a skin condition — the problem is never the condition, it is claiming to fix it.',
  ];
  ...
  // Oral care and child safety need the replacement wording, not just the ban, or the
  // retry deletes the whole fluoride explanation or the supervision advice with it.
  if (violations.some((v) => v.category === 'oral-drug-claim' || v.category === CHILD_SAFETY_CATEGORY
    || (v.category === 'disease' && /cavit|decay|caries|gingivitis|gum|periodont/i.test(v.match)))) {
    lines.push(ORAL_CARE_COMPLIANCE_RULE);
  }
  return lines.join('\n');
}
...
export const SEO_COPY_COMPLIANCE_RULE =
  ...
  'anything; never mention prescriptions, steroids or medications; never claim it is absorbed ' +
  'into the body; never claim clinical, dermatologist or FDA backing. ' +
  'You MAY name a skin condition (eczema, psoriasis, dermatitis, acne) and write about it, and ' +
  'you may say the product is SUITABLE for that kind of skin — "for eczema-prone skin", ' +
  '"gentle on irritated skin". What you may never do is claim it fixes the condition: ' +
  '"heals eczema", "eczema treatment", "clears up psoriasis" are all forbidden. ' +
  'The condition is not the problem; claiming to cure it is. ' +
  'Ingredient-avoidance language ("no SLS", "fragrance-free", "clean ingredients") is fine.\n' +
  PRODUCT_CATEGORY_COMPLIANCE_RULE + '\n' +

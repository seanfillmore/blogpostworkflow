...
//
// ── ORAL CARE, added 2026-09-13 and narrowed the same day by operator ruling ──────
//
...
// Nothing here caught it: no word in it was in the vocabulary. The `oral-drug-claim`
// category in HEALTH_CLAIM_PATTERNS now blocks that shape on every surface, like the
// therapeutic verbs, and ORAL_CARE_COMPLIANCE_RULE tells the first generation.
//
// The first version (PR #882) also blocked NAMING and a brand fact. The operator
// overruled both the same day:
//
//   · Dental conditions had joined `disease`, so a mere mention blocked on product copy,
//     in ads and in the PDP builder, and the bare noun "remineralization" blocked. Sean, on
//     "cavity-causing bacteria", "useful for customers with gingivitis" and "glycerin may
//     interfere with natural remineralization": "I don't see an issue with those
//     statements. They don't claim to cure anything." The eczema line again. Only a claim
//     that the toothpaste prevents, fights or treats a dental condition, repairs enamel or
//     supports remineralization blocks; naming the condition or the mechanism passes.
//
//   · A `child-safety` category blocked "safe for kids" / "safe to swallow". Sean: "Are
//     these products safe for kids is a resounding yes for every single product." It is a
//     brand fact, not a claim. There is no child-safety rule in this gate; do not add one.
//
// MEASURED after the narrowing, read-only, 2026-09-13, against the pre-#882 gate:
//   · 1,145 live strings on 374 resources: 5 blocking verdicts change, every one a DRAFT
//     collection body carrying a real claim ("remineralizes", "restore enamel",
//     "strengthens enamel", "remineralizing"). Zero articles, products, pages or live
//     collections change.
//   · Live theme (the daily copy-surface detector's corpus): identical to the pre-#882
//     baseline, so no acknowledged keeps are needed.
//   · Personas: 5 of 5, 15 of 15 angles, zero drops. Every oral statement in data/brand/
//     passes except "fluoride as the standard cavity-prevention chemistry", which the
//     `therapeutic` noun "prevention" has always caught.
//
...
/**
 * The standing instruction for oral care, stated in BOTH directions for the same reason
 * REGULATORY_PHRASING_RULE is: a model told only "never mention cavities" strips the
 * fluoride explanation that fluoride-free toothpaste pages exist to give. The operator's
 * line (2026-09-13) is the eczema line: naming a condition or a mechanism is information,
 * claiming our product fixes it is the drug claim. There is deliberately nothing here about
 * children — "safe for kids" is a brand fact for every RSC product, not a claim.
 */
export const ORAL_CARE_COMPLIANCE_RULE =
  'ORAL CARE (hard rule): our toothpaste is a fluoride-free COSMETIC. Say it cleans, polishes, '
  + 'freshens breath and is gentle. You MAY name cavities, tooth decay, gingivitis or gum disease, '
  + 'describe what causes them, and explain fluoride\'s role in remineralization as a CATEGORY — '
  + 'naming a condition or a mechanism is information. What you may never do is claim our '
  + 'toothpaste fixes teeth: never say it remineralizes, supports remineralization, rebuilds, '
  + 'repairs or strengthens enamel, or prevents, fights or protects against cavities, tooth decay '
  + 'or gum disease. Those are drug claims.';
...
export function seoCopyConstraint(violations) {
  ...
  // An oral claim needs the permission half too, or the retry deletes the fluoride
  // explanation and every mention of cavities along with the claim.
  if (violations.some((v) => v.category === 'oral-drug-claim')) {

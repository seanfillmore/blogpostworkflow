 ...
 *
 * FIELD NAME IS NOT CONSISTENT ACROSS FUNCTIONS — check the one you're calling,
 * never assume. The per-query-row functions below split into two camps:
 *   returns { keyword, ... }:  getTopKeywords, getPageKeywords, getLowCTRKeywords,
 *                              getPage2Keywords, getQuickWinPages
 *   returns { query, ... }:    getAllQueryPageRows, getKeywordsForDate,
 *                              getQueriesByPageForDate
 * Every current consumer destructures the right one, so there is no live bug from
 * this today — but a 2026-08 fix (agents/gsc-query-miner/leaks-feed.js) shipped
 * silently reading `query` off a `getTopKeywords`-shaped row: the field was
 * `undefined`, JSON.stringify dropped the key entirely, and the resulting feed had
 * no query text at all with nothing in the output to flag it. Check the specific
 * function's own "Returns:" docstring below rather than assuming either name.
 */

import { readFileSync } from 'fs';
import { join, dirname } from 'path';
import { fileURLToPath } from 'url';
import { gscPageUrl } from './gsc-page-url.js';
import { planOrderedQuery, applyOrder } from './gsc-order.js';
...
async function gscQuery(requested) {
  // The API has no orderBy: it always sorts by clicks. See lib/gsc-order.js.
  const { body, order, requestedLimit } = planOrderedQuery(requested);
  ...
  const data = await res.json();
  return applyOrder(data.rows || [], order, requestedLimit);
}
...
export async function getPagePerformance(pageUrl, days = 90) {
  const rows = await gscQuery({
    ...
    dimensionFilterGroups: [{
      // gscPageUrl: an exact filter on the myshopify host matches nothing and reads as 0.
      filters: [{ dimension: 'page', operator: 'equals', expression: gscPageUrl(pageUrl) }],
    }],
    rowLimit: 1,
  });
...
/**
 * getBlogPerformanceForRange(startDate, endDate)
 *
 * Blog-wide clicks/impressions/CTR over an ARBITRARY window — the control arm
 * for every meta A/B decision.
 *
 * WHY IT TAKES EXPLICIT DATES. Every other helper here is trailing-N-days from
 * today, which cannot express "the 28 days ending the day this test went live".
 * Without that window there is no way to ask what the untouched corpus did over
 * the same period, and a rewrite gets credited or blamed for it. Blog CTR
 * across all 190 pages went 0.166% → 0.505% over six consecutive 28-day blocks
 * ending 2026-08-21 with nobody touching most of them; every "improvement" the
 * A/B tracker ever recorded is smaller than that drift.
 *
 * One row, no dimensions, filtered to /blogs/ — the cheapest shape GSC will
 * answer this in.
 *
 ...
 */
export async function getBlogPerformanceForRange(startDate, endDate) {
  const rows = await gscQuery({
    startDate,
    endDate,
    dimensions: [],
  ...
  if (!rows.length) return { clicks: 0, impressions: 0, ctr: 0, position: null };
  const r = rows[0];
  return { clicks: r.clicks, impressions: r.impressions, ctr: r.ctr, position: r.position ?? null };
}

/**
 * getPagePerformanceForRange(pageUrl, startDate, endDate)
 * getPagePerformance with an explicit window, for the same reason as above.
 */
export async function getPagePerformanceForRange(pageUrl, startDate, endDate) {
  const rows = await gscQuery({
    startDate,
    endDate,
    dimensions: ['page'],
...
export async function getKeywordsForDate(date, limit = 10) {
  if (!/^\d{4}-\d{2}-\d{2}$/.test(date)) {
    throw new Error('getKeywordsForDate: date must be YYYY-MM-DD, got: ' + date);
  }
  const rows = await gscQuery({
    startDate: date,
    endDate: date,
...
export async function getPagesForDate(date, limit = 10) {
  if (!/^\d{4}-\d{2}-\d{2}$/.test(date)) {
    throw new Error('getPagesForDate: date must be YYYY-MM-DD, got: ' + date);
  }
  const rows = await gscQuery({
    startDate: date,
    endDate: date,
...
/**
 * getQueriesByPageForDate(date, limit)
 * Per-(query, page) rows for a single specific date — the query×page join
 * needed by the keyword-index builder for GSC qualification.
 * Note: GSC data lags ~3 days — pass a date 3 days in the past.
 * Returns: [{ query, page, clicks, impressions, ctr, position }]
 */
export async function getQueriesByPageForDate(date, limit = 5000) {
  if (!/^\d{4}-\d{2}-\d{2}$/.test(date)) {
    throw new Error('getQueriesByPageForDate: date must be YYYY-MM-DD, got: ' + date);
  }
  const rows = await gscQuery({
    startDate: date,
    endDate: date,

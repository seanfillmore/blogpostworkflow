...
// Overridable so tests can meter into a temp dir. Without this, any test that
// exercises a metered client appends fake token counts to the real cost report
// and silently skews `scripts/llm-cost.mjs --week`.
const USAGE_DIR = process.env.LLM_USAGE_DIR
  || join(__dirname, '..', 'data', 'reports', 'llm-usage');
...
//
// Opus was 15/75 here until 2026-07-29 — the Opus 3 / 4.0 / 4.1 rate. Every Opus
// this fleet calls (4.6, 4.7, 5) is 5/25, so every Opus cost logged before that
// date reads exactly 3× high. Matching is by family name, not version, so this
// table cannot price two Opus generations differently: if a 15/75-era model is
// ever reintroduced, priceFor needs a version check rather than a number change.
const PRICING = {
 ...
 */
export function logUsage({ model, usage, agent, ts, transport } = {}) {
  try {
    ...
    const line = {
      ...
      // 'subscription' rows are billed to the Claude plan, not per token — see
      // lib/claude-subscription.js. Rows written before the field existed are API rows.
      transport: transport || 'api',
      est_cost_usd: transport === 'subscription' ? 0 : estimateCost(model, usage || {}),
    };
 ...
 */
/**
 * Cost of one record, recomputed from its token counts rather than trusted from its
 * stored `est_cost_usd`.
 *
 * Tokens are the fact; cost is derived from a price table that has now been wrong
 * once. Repricing at read time means a table correction applies to the whole archive
 * instead of only to rows written afterwards — which matters because the spend-cut
 * decision reads weeks of history, and the Opus rows in it were booked at 3x.
 *
 * The stored value remains the fallback for any row without token counts, so an
 * older or hand-written record degrades to its recorded cost rather than to zero.
 */
function recordCost(r) {
  if (r.transport === 'subscription') return 0;
  const hasTokens = r.input_tokens != null || r.output_tokens != null
    || r.cache_write_tokens != null || r.cache_read_tokens != null;
  if (!hasTokens) return r.est_cost_usd || 0;
...
export function summarizeRecords(records = []) {
  const byAgent = {};
  const byModel = {};
  let totalCost = 0, totalCalls = 0, totalIn = 0, totalOut = 0, subscriptionCalls = 0;
  for (const r of records) {
    const cost = recordCost(r);
    totalCost += cost; totalCalls++;
    if (r.transport === 'subscription') subscriptionCalls++;
    totalIn += r.input_tokens || 0; totalOut += r.output_tokens || 0;
    (byAgent[r.agent] ||= { calls: 0, cost: 0 }).calls++;
    byAgent[r.agent].cost += cost;
  ...
  return {
    ...
    totalCalls, subscriptionCalls, apiCalls: totalCalls - subscriptionCalls, totalInputTokens: totalIn, totalOutputTokens: totalOut,
    byAgent: rank(byAgent), byModel: rank(byModel),
  };
}

/**
 * `usageDir` is injectable so callers that already take a data root (daily-summary's
 * buildDigestHtml) can stay deterministic. Reading the module-level USAGE_DIR made
 * that function's output depend on the machine it ran on.
 */
export function readUsage(dateStr, usageDir = USAGE_DIR) {
  const path = join(usageDir, `${dateStr}.jsonl`);
  if (!existsSync(path)) return [];
  return readFileSync(path, 'utf8').split('\n').filter(Boolean).map((l) => { try { return JSON.parse(l); } catch { return null; } }).filter(Boolean);
}

export function listUsageDates(usageDir = USAGE_DIR) {
  try { return readdirSync(usageDir).filter((f) => f.endsWith('.jsonl')).map((f) => f.replace('.jsonl', '')).sort(); }
  catch { return []; }
}

...
**Where strictness lives now.** The claim gate (`claims.js`) is unchanged — an unsourced factual claim still stops the run before anything renders, no override. **Product fidelity** (`verify.js`) is still a hard fail: the reference photographs go to the verifier and a bottle that contradicts them on shape, cap, label element order, printed graphics or base colour is rejected, because Photoshop cannot fix a wrong product. Photographic styling — lighting, gloss, shadow, crop — is explicitly never a mismatch; relaxing that rejects good renders at $0.13 a retry. **Stray text on a plate** is a hard fail too: you cannot remove pixels. Everything about the comp is advisory, including its 1-5 quality score, which feeds `run.json`'s `ranking[]` and the rolling baseline in `data/reports/ad-studio/scores.jsonl`. `ok: true` means checked, not good.

...
**Ad creative pipeline — two agents, two jobs.** `agents/ad-studio/` produces **finished, publish-ready static ads** end to end — copy generated and baked into the image, single generative pass, product rendered in-scene from real reference photos, nothing composited. `agents/creative-packager/` produces **placement crops from an already-approved master** (Studio one-off, or Ad Builder reference-ad-driven variations) and deliberately ships text-free images with a Photoshop-guide overlay for hand-finishing — that job is unchanged. Ad Studio's claim gate (`agents/ad-studio/claims.js`) hard-blocks any factual ad claim that doesn't quote verbatim evidence from a named source (PDP, catalog, brand kit, reviews) — no override flag, by design. Its verify gate also checks **product fidelity**: the reference photographs go to the verifier as well as the renderer, and a render whose bottle contradicts them on shape, cap, label element order, printed graphics or base colour is rejected. That check exists because a frame with a wrong bottle — no accent bar, no illustration, badge restructured — was accepted on attempt 1 when the gate only read text. Photographic styling (lighting, gloss, shadow, crop) is explicitly never a mismatch; relaxing that rejects good renders at $0.13 a retry. A **separate** art-direction call (`agents/ad-studio/critique.js`) then asks whether the frame is *usable*: it hard-fails copy that sits under Meta's UI chrome (9:16 only — 1:1 and 4:5 are not overlaid, so gating them would just burn retries) or copy illegible at thumb size, and separately records a 1-5 quality score that **never blocks**. Rank accepted frames by `critique.score` — `run.json`'s `ranking[]` puts the best one first, and every score is appended to `data/reports/ad-studio/scores.jsonl` as a rolling baseline. `ok: true` means checked, not good. **`--formats` is required and the default run is 2 renders (~$0.26): one format, one variation, Meta feed only.** 9:16 is excluded by default because all six layouts put copy in Meta's Stories/Reels UI bands and the image model keeps doing so even when told not to — 6 of 6 live attempts failed, so it is reachable via `--targets meta=9:16` but never paid for by accident. Keep that call separate from `buildVerifyPrompt` — the verify prompt's literal "do not interpret anything" framing is the opposite instruction and took five fix rounds to stabilise. See `agents/ad-studio/README.md`. The fleet's image-generation model IDs in `config/creative-models.js` are the **GA** releases, `gemini-3-pro-image` and `gemini-3.1-flash-image` — verified against the models endpoint 2026-08-14. Do not reintroduce the `-preview` suffixes; those are older, separately-billed endpoints. `data/creatives/ad-studio/` run output is gitignored and unbounded by default (one run ≈137 MB); `scripts/prune-ad-studio.mjs` (`npm run prune-ad-studio`) prunes rejected-variation images after a 7-day grace period and all images past a 90-day retention window, keeping every JSON file (the audit record) forever. Dry-run by default, `--apply` to delete — see `agents/ad-studio/README.md`'s Housekeeping section.

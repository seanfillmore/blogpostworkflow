...
**`--formats` is required.** The default run is one format, one variation, all three Meta placements. Demand Gen plates are opt-in (`--targets all`). The fleet's image-generation model IDs in `config/creative-models.js` are the **GA** releases, `gemini-3-pro-image` and `gemini-3.1-flash-image` — verified 2026-08-14. Do not reintroduce the `-preview` suffixes. `data/creatives/ad-studio/` is gitignored and unbounded; `scripts/prune-ad-studio.mjs` (`npm run prune-ad-studio`) prunes rejected images after 7 days and all images past 90, keeping every JSON forever. Dry-run by default, `--apply` to delete. See `agents/ad-studio/README.md`.
